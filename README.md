...
> **计划调整（2026-07-06）**：应公司要求，整体以 **7/31（周五）为 end date**，比原定八周提前两周。剩余排期从「6 周内容」收口为 **4 周**（W3–W6）。做法是**按优先级砍范围、保深度**，而非把内容前置挤压——挤压会让每周变浅，破坏「每周可演示 demo + 能脱离 AI 从空白重建」的验收标准。BE 主干（查询 / 认证 / 底层 / 测试）保持全深度；**AI 能力整合、全栈前端 demo 移出本期**（详见文末 backlog）。
...
- [ ] 第 6 周（7/27–7/31）：测试与工程化 + 复盘收尾
- [ ] ~~AI 能力整合~~ → 移出本期（见文末 backlog）
- [ ] ~~全栈整合（前端 demo）~~ → 移出本期（复盘并入第 6 周）
...
| 6 | 7/27–7/31 | 测试与工程化 + 复盘收尾 | 单元/集成测试、CI；整体技术总结 | 测试套件 + CI 跑通 + 技术总结 |

> 并行线：每周阅读 1–2 篇英文技术文档 / 写一段英文技术总结。

### 移出本期的内容（backlog，7/31 后再评估）

砍范围而非压深度，以下两块离「BE strong」目标最远，最先移出：

- **AI 能力整合**（原 W7）：LLM API 集成、RAG、流式响应。附加能力，非本期核心，整体延后。
- **全栈整合 · 前端 demo**（原 W8）：端到端串联 + 轻量前端界面。FE 仅要求 general 水平，前端搭建成本高、对后端能力证明贡献低，本期移除；其中的**复盘 / 技术总结**保留并并入第 6 周收尾。

> 对应的空目录 `week7-ai/`、`week8-fullstack/` 暂保留占位，不在本期排期内。
