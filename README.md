...
- [ ] 第 5 周（7/20–7/27 收口）：Node.js 底层原理（核心） ← 当前（周末不补习，7/27 首个完整专注块完成 D6 闸门）
- [ ] 第 6 周（7/27 D6 通过后–7/31）：测试与工程化 + 全栈整合 demo + 复盘收尾
...
| 6 | 7/27 D6 通过后–7/31 | 测试与工程化 + 全栈整合 + 复盘 | 单元/集成测试、CI；端到端串联；整体技术总结 | 测试套件 + CI 跑通 + 全栈 demo（前端轻量）+ 技术总结 |
