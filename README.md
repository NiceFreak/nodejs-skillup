...
**第 6 周内部节奏（收尾周较满，按这个顺序切）：**
1. **7/27–7/30 测试与工程化**（本周核心学习产出，测试用例由本人自己写）：单元/集成测试 + CI 跑通。
2. **7/31 全栈整合 + 复盘**（capstone，主要是把已有各周产出接成一条端到端链路）：前端基础验收 UI 由 AI 搭脚手架（`AGENTS.md` 例外条款允许），本人负责串联与讲清整体架构；同日产出**技术总结 / 复盘**，其中点明整个 skillup 过程如何借助并驾驭 AI。
