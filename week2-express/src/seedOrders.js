...
const HOUR_WEIGHTS = [
    2, 1, 1, 1, 1, 1, 2, 4, 6, 8, 9, 9,    // 0~11
    10, 8, 8, 9, 9, 10, 12, 15, 16, 14, 9, 5, // 12~23
];
...
function intensity(t) {
    ...
    const day = d.getDate();
    if (m === 11 && day >= 10 && day <= 12) f *= 6;       // 双11
    else if (m === 6 && day >= 17 && day <= 19) f *= 5;   // 618
    else if (m === 12 && day >= 11 && day <= 13) f *= 3;  // 双12
...
function buildOrdersForUsers(users, seed = RANDOM_SEED) {
    ...
    const drawStatus = (createdAt) => {
        ...
        if (ageDays < 2) {
            return weightedPick([["pending", 55], ["completed", 30], ["canceled", 10], ["refunding", 5]]);
        }
        if (ageDays < 14) {
            return weightedPick([["completed", 62], ["pending", 8], ["canceled", 15], ["refunding", 5], ["refunded", 10]]);
        }
        return weightedPick([["completed", 76], ["canceled", 12], ["refunded", 10], ["pending", 1], ["refunding", 1]]);
    };

    const buildItems = () => {
        // 一单里不同商品的数量：以 1 种为主
        const distinct = weightedPick([[1, 60], [2, 25], [3, 10], [4, 5]]);
        const items = [];
        let totalAmount = 0;
        ...
        for (let k = 0; k < distinct; k++) {
            ...
            // 购买件数：以 1 件为主
            const quantity = weightedPick([[1, 70], [2, 20], [3, 7], [4, 2], [5, 1]]);
            // 价格轻微浮动（促销/尾数），保留两位小数
...
function summarize(orders, userCount) {
    ...
    console.log(`   订单总数: ${orders.length}  |  下过单的用户: ${buyers.size}/${userCount}  |  零单用户: ${userCount - buyers.size}`);
    console.log(`   人均订单(下单用户): 中位数 ${q(perBuyer, 0.5)} / P90 ${q(perBuyer, 0.9)} / 最多 ${perBuyer[perBuyer.length - 1] || 0}`);
    console.log(`   客单价(AOV): 中位数 ¥${q(amounts, 0.5)} / P90 ¥${q(amounts, 0.9)} / 最高 ¥${amounts[amounts.length - 1] || 0}`);
    console.log(`   总 GMV: ¥${Math.round(revenue).toLocaleString()}`);
    console.log("   各状态占比:", Object.fromEntries(Object.entries(byStatus).map(([k, v]) => [k, `${((v / orders.length) * 100).toFixed(1)}%`])));
