...
- few-shot examples：无
- 变更理由：v0 的 §1（唯一发送给模型的 system instructions）只提到 answered/claims/citations 这些概念词，
  未规定响应 JSON 的键名；§3 中写明键名的示例被冻结决定限定为不发送给模型。首次真实调用因此返回 `status`、
  顶层 `citations` 与 `schema_version`，落 `schema_error`。v1 在 §1 增加响应键契约；其余内容不变。
  触发证据：`week13-rag/evidence/baseline/smoke-dev-01.json`。
