...
> 证据：`evidence/bm25-e2e/dev-bm25-e2e-top10-01.json`（`bm25-end-to-end`，10 题）
...
- `source_span` requirement 共 **16** 项：**2** 项按 R1 ② 已由机械确定未覆盖；**1** 项属无答案题的 advisory，不参与该题通过判定（契约 §1）；**5** 项不适用（该题响应无 claims，按 §2 / §3 的机械条件判定）。
- 含至少一项已确定未覆盖的 item：**2** 题（`w13-dev-direct-answer-01`, `w13-dev-paraphrase-01`）。
- 本次响应共有 **26** 条 claim 需要本人逐条判断支持关系。
...
| 2 | `rules/AGENTS.md#L71-L76` | 是 | 否 | 是 |
| 3 | `rules/AGENTS.md#L110-L111` | 是 | 否 | 是 |

**仍需本人判定（语义；本文件不给出结论）**

- [ ] 逐条 claim（共 3 条）是否被其 citation 指向的原文支持，且是否为 atomic claim：
...
| requirement | span | 机械结论 | span 内的 claim |
|---|---|---|---|
| R1 `该来源范围必须直接规定官方术语与类比或助记的并列关系，以及不得以自创指称` | `TECHNICAL-WRITING-PROTOCOL.md#L118-L120` | 已覆盖（机械；① 仍需人工） | [1, 2, 3, 4, 5] |
...
| 4 | `rules/TECHNICAL-WRITING-PROTOCOL.md#L120-L120` | 是 | 是 | 是 |
| 5 | `rules/TECHNICAL-WRITING-PROTOCOL.md#L120-L120` | 是 | 是 | 是 |
...
| requirement | span | 机械结论 | span 内的 claim |
|---|---|---|---|
| R1 `该来源范围必须规定讲解中的官方术语、类比或助记标注要求，并明确未标注的自` | `AGENTS.md#L161-L163` | 已覆盖（机械；① 仍需人工） | [1] |
| R2 `该来源范围必须独立规定官方术语与类比或助记的并列、标注和后续指称边界。` | `TECHNICAL-WRITING-PROTOCOL.md#L116-L120` | 已覆盖（机械；① 仍需人工） | [2, 3] |

**citation 预筛（可解析 / 落在 requirement 内 / 与本次 context 相交）**

| claim | citation | 可解析 | 落在某 requirement 内 | 与本次 hits 相交 |
|---|---|---|---|---|
| 1 | `rules/AGENTS.md#L163-L163` | 是 | 是 | 是 |
| 2 | `rules/TECHNICAL-WRITING-PROTOCOL.md#L120-L120` | 是 | 是 | 是 |
| 3 | `rules/TECHNICAL-WRITING-PROTOCOL.md#L118-L118` | 是 | 是 | 是 |
...
| requirement | span | 机械结论 | span 内的 claim |
|---|---|---|---|
| R1 `该来源范围必须表明技术学习文案规范适用于学习展板的用户可见文案，并说明其` | `TECHNICAL-WRITING-PROTOCOL.md#L1-L5` | 已覆盖（机械；① 仍需人工） | [1] |
| R2 `该来源范围必须说明学习展板视觉规范负责视觉形态与验收，并区分技术文案规范` | `SHOWCASE-VISUAL-PROTOCOL.md#L1-L10` | 已覆盖（机械；① 仍需人工） | [2] |

**citation 预筛（可解析 / 落在 requirement 内 / 与本次 context 相交）**

| claim | citation | 可解析 | 落在某 requirement 内 | 与本次 hits 相交 |
|---|---|---|---|---|
| 1 | `rules/TECHNICAL-WRITING-PROTOCOL.md#L3-L3` | 是 | 是 | 是 |
| 2 | `rules/SHOWCASE-VISUAL-PROTOCOL.md#L3-L3` | 是 | 是 | 是 |
| 3 | `rules/TECHNICAL-WRITING-PROTOCOL.md#L182-L183` | 是 | 否 | 是 |
| 4 | `rules/AGENTS.md#L171-L173` | 是 | 否 | 是 |
| 5 | `rules/LEARNING-PROTOCOL.md#L45-L46` | 是 | 否 | 是 |
...
| R2 `该来源范围必须定义 L1 的讲解边界，并规定黑名单最高援助等级为 L2。` | `AGENTS.md#L101-L110` | 已覆盖（机械；① 仍需人工） | [2] |
...
| 2 | `rules/AGENTS.md#L106-L106` | 是 | 是 | 是 |
| 3 | `rules/AGENTS.md#L260-L262` | 是 | 否 | 是 |
| 4 | `rules/AGENTS.md#L260-L262` | 是 | 否 | 是 |
| 5 | `rules/LEARNING-PROTOCOL.md#L223-L223` | 是 | 否 | 是 |
...
| requirement | span | 机械结论 | span 内的 claim |
|---|---|---|---|
| R1 `该来源范围必须将事件循环等 Node.js 底层机制及其理解型 demo` | `AGENTS.md#L66-L78` | 已覆盖（机械；① 仍需人工） | [1] |
| R2 `该来源范围必须定义 L1 原理讲解，并规定黑名单最高援助等级为 L2。` | `AGENTS.md#L101-L110` | 已覆盖（机械；① 仍需人工） | [4] |

**citation 预筛（可解析 / 落在 requirement 内 / 与本次 context 相交）**

| claim | citation | 可解析 | 落在某 requirement 内 | 与本次 hits 相交 |
|---|---|---|---|---|
| 1 | `rules/AGENTS.md#L77-L77` | 是 | 是 | 是 |
| 2 | `rules/AGENTS.md#L86-L86` | 是 | 否 | 是 |
| 3 | `rules/AGENTS.md#L11-L12` | 是 | 否 | 是 |
| 4 | `rules/AGENTS.md#L105-L105` | 是 | 是 | 是 |
| 5 | `rules/AGENTS.md#L3-L3` | 是 | 否 | 是 |
...
| requirement | span | 机械结论 | span 内的 claim |
|---|---|---|---|
| R1 `该来源范围必须区分构建、部署和合并上线的权限层级，并要求按用户授权范围执` | `SHOWCASE-DEPLOY-PROTOCOL.md#L34-L38` | 不适用（本次响应无 claims；该题按契约 §2 / §3 的机械条件判定） | — |
| R2 `该来源范围必须规定发布分支需要明确，且功能分支构建成功不等于线上已经更新` | `SHOWCASE-DEPLOY-PROTOCOL.md#L60-L61` | 不适用（本次响应无 claims；该题按契约 §2 / §3 的机械条件判定） | — |
| R3 `该来源范围必须列出发布前仍需完成的范围确认、检查、构建和验证步骤。` | `SHOWCASE-DEPLOY-PROTOCOL.md#L64-L90` | 不适用（本次响应无 claims；该题按契约 §2 / §3 的机械条件判定） | — |
