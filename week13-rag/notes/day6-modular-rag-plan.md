...
| 模块 | 状态 | 证据 | 未完成原因 / 下一入口 |
|---|---|---|---|
| M0 | 已完成（离线复核） | `demo-replay.py summary/verify`、dense equivalence、工作区状态 | 本人页面复述仍待进行 |
| M1 | 材料已具备，运行待本人 | `day6-rag-code-walkthrough.md`、D5 BM25/dense e2e 证据 | 本轮未新增模型调用；需本人选择成功/失败案例复述 |
| M2 | 已完成（文档 review） | `day6-rag-code-walkthrough.md` 第 2 节、84 tests | 框架职责理解需本人确认 |
| M3 | 待执行 |  |  |
| M4 | 待执行 |  |  |
| M5 | 待执行 |  |  |
| M6 | 待执行 |  |  |
| M7 | 材料已完成，人工演练待执行 | `RAG 如何构建` 主 tab、4 个新增独立专题、`rehearse:rag --check` 80 项通过 | 全量 `verify:board` 在既有 W10 `.w10-grade-count` 等待处超时；本人计时与移动端验收待做 |
