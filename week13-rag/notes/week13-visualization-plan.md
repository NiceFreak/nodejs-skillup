...
- 不引入图标库或图表库依赖。
- 不要求五块同时交付。若周末时间只够一半，按 §14.7 确认的最小集交付；展板的验收单位是「块」，
  半块无法按 §12 验收，宁可少做一块。
...
1. §4 的五个模块组划分是否成立；T4「评测契约」在没有运行结果的情况下是否单独成块，还是压缩为 T5 的折叠层。
2. §6 各块 ② 列的主结论措辞：事实强度与边界是否准确，尤其 T5「职责边界没有自动化断言」。
...
4. T4 的合成响应样例内容——它不属于任何正式题集，但仍是判分语义的展示，语义由本人冻结。
5. 各块 ⑦ 列的视觉记忆点是否来自技术关系，是否需要替换。
6. §12 的度量护栏是否按块放宽：T2 的 parser 规则较多，默认态可能超过 1.5 屏，需要本人决定是拆页还是记录
   不可下沉的理由。
7. 五块的最小可交付集。候选：T5 + T3（一块讲「证明了什么」、一块讲「模型实际看到什么」，各自可独立成立）。
8. 六道复习题的题面与正确答案。它们是判据语义、不是文案，AI 不代填答案；本方案只给出题面候选（§6 各块 ⑥ 列）。
9. C1 与 C5 对照旁注的措辞（§9 末段）：目前只写「7 份样本呈强相关，r = 0.95，每 block 82–162 bytes」这条
   可核对的观察，未给技术结论；是否要在此处给出结论由本人决定。
10. 展板是否在展示状态可见。语料是本仓库公开规则文档、不含凭据与可定位端点，本方案倾向展示状态可见；
    最终由本人裁定。
