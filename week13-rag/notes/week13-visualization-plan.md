...
>
> 修订记录：2026-09-09 第一稿经独立审计后修订。修订项为审计指出并由复算确认的 10 处：瀑布基线与增减方向
> （§6.4、§9 C2）、五阶段枚举口径（§3.5）、normalization 不属于校验步骤（§6.2）、corpus 校验证据缺口
> （§7.4）、T5 矩阵坐标改为验证手段 × 被验证对象（§6.6）、C1/C5 对照结论删除并改为可核对观察（§9）、
> 标题层数与 `context_spans` 条数的区分（§6.3、§9 C4）、C8 阈值只对两条门禁 metric（§9）、
> §1.7 冻结基准的过期时态、② 列拆分为主结论加边界短句（§6）。
...
- 因此本方案的六块是 `ai-engineer` 板内的**功能模块组**（`AeGroup` 级），不是顶层 `ShowcaseTab`；
  为避免与代码里已有的 `ShowcaseTab` 混指，全篇不用「tab」称呼这六块。
- 形态一律按 §5 从内容关系推导。既有 `framePlayer` / `charts` / 泳道 CSS 只在 §11 作为实现复用候选出现，
  不作为形态依据（视觉规范 §1 护栏、§3 形态推导纪律）。

### 3.5 五阶段枚举（全篇复用，不得另起一套）

展板对管线只用这一组枚举，与 §2 的数据流句一致；corpus snapshot 是喂给管线的**冻结输入**，不是其中一段。

| # | 阶段 | 所属层 | 截至 2026-09-09 的状态 | 状态的证据 |
|---|---|---|---|---|
| S1 | query | 输入 | 已冻结（dev 10 题） | `eval/dev/items.json` 结构校验 |
| S2 | retrieval | 确定性 | 未实现 | 无产物 |
| S3 | context assembly | 确定性 | 已实现并自测 | 9 条测试；two-pass byte-identical；整串 sha 与冻结基准一致 |
| S4 | generation | 概率 | 未运行 | 无产物 |
| S5 | citation 校验与判分 | 确定性检查 + 人工语义 checklist | 未运行 | 无产物 |

冻结输入 `rules-c0a4b85` 的证据是 manifest 逐文件 bytes / sha256 / git blob 比对，**不是 two-pass**；
two-pass byte-identical 只覆盖 S3（`tests/test_registry_real.py::test_real_corpus_two_pass_is_byte_identical`）。
两者不能互相顶替。
...
| H7 | 「未测量」（没跑过）与 `N/A`（跑过但分母为零）是两个状态，不合并渲染 | 判分契约 §5：分母为零记 `N/A`，且不得记为通过 |
...
|---|---|---|---|---|
| T0 | §3.5 的五阶段构成固定顺序的管线；每段分属确定性层或概率层；状态分四档，断点位置本身是结论 | 顺序、归属、停止位置 | 双带管线拓扑：上带确定性层、下带概率层，冻结输入从左侧接入 S3，未实现处物理断开 | ②方位过渡。不做过程动画：S2 与 S4 未实现，播放一次「数据流过全链路」会呈现未发生的事实 |
| T1 | 一次完整性校验按固定顺序发生：读取快照文件 → 计算 bytes/sha256/git blob → 与 manifest 逐文件比对 | 顺序、逐项对照、数量差异 | 文件条（共用基线长度编码）+ 右挂逐文件比对格 + 三步校验序列 | ①语义过程动效：校验是真实按序执行的三步过程，逐帧一步。normalization 不在这条时间轴上，它发生在建快照时（见 §6.2 ⑥） |
...
| T5 | 不同验证手段各自只覆盖一部分输出对象；`content_sha256` 只覆盖 model_content，wrapper 与块序靠 fixture 期望字节补上，职责边界目前没有自动化断言 | 二维分类、覆盖空格 | 覆盖矩阵：验证手段 × 被验证对象，空格即该手段管不到的地方；旁挂门禁阶梯标出当前停在哪一级 | ②方位过渡。覆盖关系是静态事实；破坏性验证（改 parser → verify FAIL）在 9/9 未执行，只作折叠条目并标「未执行」，不演成过程 |
...
| 列 | 内容 |
|---|---|
| ① 单一问题 | §3.5 的五个阶段里，哪些已经是确定性可重跑的，哪些还没有运行过？ |
| ② 10 秒结论 | **主结论**：五段管线中只有 context assembly 已实现并双跑一致，其余三段未实现或未运行。**常驻边界**：① 冻结输入的证据是逐文件比对，不是双跑；② 未运行的阶段不存在质量结论，也不能预先归因为 retrieval miss |
| ③ 对象与数据形状 | §3.5 的 5 个阶段节点 + 1 个冻结输入节点，每个带：所属层、状态四档（已冻结 / 已实现并自测 / 未实现 / 未运行）、产物路径、状态证据。四档计数由数据计算 |
| ④ 结论编码 | 位置（上带 = 确定性层，下带 = 概率层）+ 连接（阶段顺序）+ 线型（实线 = 已可重跑，虚线 = 未运行）+ 断点位置（未实现处物理断开）。颜色只作第二编码 |
| ⑤ 视觉舞台 | 首屏：一条横贯的双带管线占据舞台，冻结输入从左侧接入 S3，断点在 S2 与 S4；管线正上方主结论；右侧四档状态计数 |
| ⑥ 文字层级 | 常驻：阶段名、状态标签、两条边界短句；折叠：每阶段产物路径与复算命令、§3.5 的状态证据列；notes：D1/D3 原文。复习题「retrieval 未实现时，为什么不能把一次回答错误归因为 retrieval miss？」 |
| ⑦ 视觉记忆点 | 实线在确定性带上贯通、进入概率带即断开——断口位置就是当前能力边界 |
| ⑧ 图标策略 | 状态四档各一个语义符号（锁 = 已冻结、对勾 = 已实现并自测、空心圆 = 未运行、断口标记 = 未实现）+ 文字名称，符号数与档数一致；无装饰图标 |
| ⑨ 动效策略 | ②方位过渡：点击阶段跳到对应模块组并高亮，过渡短、不循环。不做过程动画，理由见 §5 |
| ⑩ 验收证据 | 验收句：哪两段没有运行过？两视口截图；结构断言：未运行阶段的 DOM 不含任何数值型分数节点；图例符号数 === 状态档数；`data-anchor` 标在断点处 |
...
| ③ 对象与数据形状 | 7 文件 × (bytes、chars、行数、sha256 前 8 位)；一次校验 3 步；1 个 token 估算值 + 分类 `estimate` + tokenizer 版本。**校验结论只有一条整体记录**（2026-09-07 全部通过），仓库内没有逐文件通过状态的可复算产物，因此不做逐文件状态格 |
...
| ③ 对象与数据形状 | 一段真实源文档的连续行（含标题、表格、`---`）；标题栈层级；6 类 block 计数（list_item 349 / paragraph 144 / table_row 43 / code 23 / quote_para 10 / quote_list 3）；**标题层数**分布 4 档（1 层 15 / 2 层 391 / 3 层 156 / 4 层 10）；43 个 table_row 块各另带 1 条 `table_header` 语境；7 份文档各自 block 数 |
...
| ⑥ 文字层级 | 常驻：主结论 + 两条边界；折叠：六条切分规则原文（列表按顶层项、表格按数据行附表头、fence 只向前合并、blockquote 递归、`---` 硬边界）、review 工作表 A1–A8 的开放问题；notes：D2 原文。复习题「附加标题进入 model_content，为什么不进入 source_id 的行范围？」 |
...
### 6.4 T3 · 组装与容量：Evidence Context 的组成

| 列 | 内容 |
|---|---|
| ① 单一问题 | 进入模型的 Evidence Context 由哪几部分组成，各部分分别有多大？ |
| ② 10 秒结论 | **主结论**：进入模型的 89,854 chars 里，只有 32,171 来自核心正文。**常驻边界**：① 另外 20,826 来自把必要标题与表头复制进每个块，36,857 来自 wrapper 与块间空行；② 单位是字符，不是 token；serialized 输入的 token 计量未执行 |
...
| ⑤ 视觉舞台 | 首屏：瀑布图占主位（四级量 + 一降两升），主结论在其正上方；右侧并排单个 entry 的四形态。**容量三项（reserved output 4096 tokens、safety margin 100,000 tokens、budget 未冻结）不进首屏**——它们是 token 单位，与 chars 同屏会诱发跨单位换算（H3） |
| ⑥ 文字层级 | 常驻：主结论 + 两条边界 + 单位标注；折叠：容量三项（单独成组并标「token，与上方 chars 不可换算」）、wrapper 精确字节规则、块间恰一个空行与首尾无空行、hash 只覆盖 `model_content`、`---` 与空行为何从不进入核心；notes：D3 §6.2.0 原文。复习题「块间空行属于哪一层，为什么它不进入 `content_sha256`？」 |
...
| ⑩ 验收证据 | 验收句：89,854 里有多少来自核心正文？断言：① 四级量与三段增减全部由 registry 与 Evidence Context 产物计算，无硬编码；② 32,171 + 20,826 === 52,997 且 52,997 + 36,857 === 89,854（构建期自检）；③ 首屏 DOM 不含 token 单位数值；两视口截图 |
...
| ② 10 秒结论 | **主结论**：一题要同时满足全部条件才通过；预期 abstained 却返回 answered 直接否决整个 split。**常驻边界**：① dev 五类行为各 2 题（8 题预期 answered、2 题预期 abstained，共 18 条 evidence requirements）；② 6 个 metric 目前全部未测量，其中 4 个是诊断、没有阈值 |
| ③ 对象与数据形状 | 5 类行为 × 2 个 split 的覆盖矩阵，**格内编码该类的预期分支**（no_answer 行为 abstained，其余四行为 answered）；answered 8 条 / abstained 5 条判分条件的顺序链；6 个 metric，其中门禁 2 个带阈值（item_pass_rate ≥ 0.9、citation_precision = 1.0）、诊断 4 个无阈值 |
...
| ⑥ 文字层级 | 常驻：主结论 + 两条边界；折叠：8 条 answered 条件与 5 条 abstained 条件原文、`corpus_absence` 是评测者预先冻结的判分依据、`N/A` 与「未测量」的区别；notes：判分契约原文。复习题「citation_precision 阈值为 1.0，为什么 claim 零 citation 时它不能记为通过？」 |
| ⑦ 视觉记忆点 | 判分链上那个停止标记的位置——否决不在链尾，而在分支判定处 |
| ⑧ 图标策略 | 锁形标记表示受保护 split（accessible name「内容受保护，不展示」）；停止标记表示否决出口；其余用文字 |
| ⑨ 动效策略 | ①语义过程动效：单题判分逐帧推进一个条件并显示通过 / 失败。样例响应为合成数据，帧内常驻标注「契约演示，非模型运行结果，不属于任何正式题集」。可暂停、单步、重放 |
| ⑩ 验收证据 | 验收句：哪一个条件失败会否决整个 split？断言：① 受保护 split 的列在 DOM 中只有计数与状态、无题面字段；② 6 个 metric 均渲染为「未测量」而非 0，且 4 个诊断 metric 的阈值位渲染「无阈值（诊断）」；两视口截图 |

### 6.6 T5 · 门禁与证据：验证手段的覆盖与空白

| 列 | 内容 |
|---|---|
| ① 单一问题 | 每种验证手段各自管到哪里，哪些对象目前没有自动化断言？ |
| ② 10 秒结论 | **主结论**：已证明的是确定性组装层可重跑并与冻结基准一致；职责边界（判据 #7）目前没有自动化断言。**常驻边界**：① 9 条测试与覆盖审计只覆盖确定性层，不证明语义切分合理、retrieval 有效或模型回答正确；② wrapper 前置检查发生在构建期，不在 `tests/` 里 |
| ③ 对象与数据形状 | 覆盖矩阵 = 6 种验证手段（`content_sha256` / fixture 期望字节 / 结构不变式 / two-pass / 构建期前置检查 / 无自动化）× 5 类被验证对象（`model_content` 正文 / wrapper 字节 / 块序与块间分隔 / registry 契约（重复 ID、排序）/ 职责边界）；9 条测试（5 fixture + 4 真实语料）；7 份文档 uncovered 与 duplicated 均为 0；门禁阶梯 5 级及当前档位 |
| ④ 结论编码 | 矩阵（验证手段 × 被验证对象）+ 覆盖空格（`content_sha256` 行只有一格有值；「职责边界」列只落在「无自动化」行）+ 停止位置（门禁阶梯当前档） |
| ⑤ 视觉舞台 | 首屏：覆盖矩阵占主位，`content_sha256` 那一行的空格清晰可见；右侧门禁阶梯竖排 5 级并标当前档；主结论在矩阵上方 |
| ⑥ 文字层级 | 常驻：主结论 + 两条边界；折叠：判据 #1–#7 逐字原文与它们各自的观察对象、A1–A8 review 工作表状态、破坏性验证方案（标「未执行」）；notes：D3 §6.1 原文。复习题「fixture 期望字节能抓到哪一类 `content_sha256` 抓不到的错误？」 |
...
| ⑩ 验收证据 | 验收句：`content_sha256` 管不到什么，被什么补上？断言：① 矩阵格状态由数据层计算；② 「职责边界」列在「无自动化」行有值；③ 破坏性验证条目带「未执行」状态字段；两视口截图 |
...
（`已实测` / `已估算` / `一次性记录` / `待运行验证`）、`sourcePath`、`status`
（`frozen` / `implemented` / `not_implemented` / `not_run`）。

- 未跑过的项 `value` 为 `null`，渲染「未测量」。**跑过但分母为零**的项 `value` 为字符串 `"N/A"`，
  渲染「N/A（分母为零）」，与「未测量」不同格（硬边界 H7；判分契约 §5 规定 `N/A` 不得记为通过）。
- 有阈值的 metric 才带 `threshold` 字段；诊断 metric 不设该字段，渲染「无阈值（诊断）」。
- `status` 取值随主线推进变化，实现时以当时 `LEARNING-STATE.md` 为准；本方案记录截至 2026-09-09 的取值。
...
### 7.4 corpus 完整性校验的证据缺口

D1 笔记记录「manifest 汇总与当前内容的逐文件字节/SHA-256/Git blob 校验均通过」（2026-09-07），但仓库内
没有可重跑的校验命令，也没有落盘的逐文件结果——`scripts/` 只有 `w13rag.sh` 与 `inspect-block.sh`，
`cli.py` 只从 manifest 取 `corpusId`/`snapshotId` 与文档顺序，不复算 bytes 或 sha256。

因此 T1 当前按「一次性记录」呈现：不做逐文件通过状态格，不在动效末帧显示实时通过态。
是否补一条只读校验命令并落盘 JSON 证据（补上后 §7.1 增行、T1 恢复逐文件状态格），列入 §14 待确认项。
...
| C6 覆盖矩阵 | T5 | 6 验证手段 × 5 被验证对象 | 矩阵 + 覆盖空格 | 形状 + 文字（已覆盖 / 该手段管不到） |
| C7 eval 覆盖矩阵 | T4（尺寸小于判分链） | 5 行为 × 2 split，格内编码预期分支 | 矩阵，受保护列上锁 | 锁形 + 分支文字 |
| C8 metric 状态条 | T4 | 6 metric × (阈值?, 当前状态) | 门禁 2 条带阈值刻度；诊断 4 条阈值位渲染「无阈值（诊断）」 | 「未测量」/「N/A（分母为零）」文字，不画 0 值柱 |
...
- 数据导出脚本置于 `week13-rag/scripts/`，只读 corpus 与 evidence 产物，输出 TS 数据模块；不调用模型，
  不读取受保护 split 内容。该目录的现有脚本与输出字段含义见 `week13-rag/scripts/README.md`；
  导出脚本的 block 类型计数取自 `criteria-report-*.md`，不要去 registry 找不存在的 `kind` 字段。
...
| 度量 | 每块记录默认态屏数（目标 ≤ 1.5 屏）、全部展开态屏数、主路径中文正文字数（目标 ≤ 220 字，超 300 字须说明哪些限定语不能下沉）。§6 的 ② 列已按「主结论 ≤ 40 字 + 常驻边界短句」拆分，实现后按实际渲染文本重新计数，不沿用设计期估计 |
...
- 不把 W13 的可迁移思维模式（§1 的八条）单独做成一块板；它们作为各功能 tab 的折叠层结论出现，避免与
  技术对象脱钩成为独立方法论页。
- 不引入图标库或图表库依赖。
- 不要求六块同时交付。若周末时间只够一半，按 §14.8 确认的最小集交付；展板的验收单位是「块」，
  半块无法按 §12 验收，因此宁可少做一块，不做六个半成品。
...
7. 展板是否在展示状态可见。语料是本仓库公开规则文档、不含凭据与可定位端点，本方案倾向展示状态可见；
   最终由本人裁定。
8. 六块的最小可交付集。候选：T5 + T3（一块讲「证明了什么」、一块讲「模型实际看到什么」，各自可独立成立）；
   由本人确认最小集与优先级。
9. 六道复习题的题面与正确答案。它们是判据语义、不是文案，AI 不代填答案；本方案只给出题面候选（§6 各块 ⑥ 列）。
10. §3.5 五阶段枚举的口径：corpus snapshot 作为「冻结输入」而不是管线中的一段，是否符合本人对失败归因分层的
    定义；若改，T0 的 ②③⑤ 需同步回写。
11. 是否补一条 corpus 完整性只读校验命令并落盘 JSON 证据（见 §7.4）。补：T1 恢复逐文件状态格与「7/7 通过」
    的可复算结论；不补：T1 维持「一次性记录」呈现。这决定 T1 的 ③⑤⑨⑩ 四列。
12. C1 与 C5 对照旁注的措辞（§9 末段）：目前只写「7 份样本呈强相关，r = 0.95，每 block 82–162 bytes」这条
    可核对的观察，未给技术结论；是否要在此处给出结论由本人决定。
