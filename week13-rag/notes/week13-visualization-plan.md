...
> 建立：2026-09-09（Asia/Shanghai）。状态：**返工完成（2026-09-10）：T0 总览已补，五块主图已对象化；缘由与逐块改动见 §3.7 / §3.7.1。待本人验收**。
...
## 3.7 返工：先建立对象，再讲它的内部（2026-09-10）

**收到的问题**：五块交付后，本人的判断是「图表和文字很脱节，不知道这个可视化展示的是什么」。

**复核后确认成立，defect 很具体**：五块的图编码的是**量与分类**（char 数、块数、格数），
而每块的结论讲的是**机制**（身份与指纹分离、硬边界、覆盖盲区）。两者不在回答同一个问题。
更根本的是，图上所有标签都是内部术语（`核心 source_span`、`model_content`、`wrapper`、
`fixture 期望字节`、`two-pass`），这些词在页面上从未被任何视觉形式定义过——读者要先读完折叠层才能看懂主图，
与「结论由版面承担」正好相反。

**根因是 §3.5 删除 T0 的那条决定**。当时的理由是「五段管线有三段无产物，一块只讲什么没做的板不成立」。
这个理由站不住：总览的职责不是汇报完成度，而是**建立读者对这个对象的认识**——一份规则文档如何变成
模型可引用的证据、引用如何被判对错。删掉它之后，其余五块全部在讲这个对象的内部统计，而读者从没见过对象本身。

**修正（本人 2026-09-10 裁定，采用推荐方案）**：
1. 补一块 T0 链路总览，用一条真实内容走完 ①冻结文档 → ②切块 → ③组装 → ④作答 → ⑤回查判分，
   术语在图上由真实内容定义一次。
2. 五块的主图从统计图改为对象图：画真实的块与真实的字符串，量退成标注。
3. 导航顺序改为跟随链路（总览 → ① → ② → ③ → ⑤ → 贯穿的验证），每块顶部标出它是总览哪一段的展开，
   并提供一键回总览。

T0 不违反 §3.5 的「未完成不进板」：它画的是**已冻结并已实现的契约链路**，retrieval 与 generation
以一句边界文字处理（本次全语料、不做检索；第 ④ 步是合成数据），不占版面、不画成阶段。

### 3.7.1 五块主图的对象化改造（2026-09-10 完成）

| 块 | 原主图（统计） | 新主图（对象） | 关键改动 |
|---|---|---|---|
| T1 输入冻结 | 7 根按文件大小排的条 + 21 个状态格 | 一份文件展开成三项「manifest 记录值 vs 本次复算值」两列对照，其余六份各三格 | 删掉文件大小条形——条长编码体量，与「完整性怎么被证明」无关，属装饰。结论「逐文件、逐项」由两列并置 + 七份都在场承担 |
| T2 切分与引用 | 真实源行 + 标题栈（已是对象图） | 不变 | 这一块本来就以真实内容为主图，只做了语境行的可读性修正 |
| T3 组装与组成 | 七根瀑布条为主图，真实 entry 为配角 | 真实 entry 逐层长大为主图（全宽、可读），全语料瀑布退到其下并写明「上面那一个块，在全语料 572 个块上合计起来是这样」 | 主次对调；结论锚随之从瀑布移到 entry |
| T4 评测契约 | 8 条打勾的编号列表 + 十格都写「2 题」的矩阵 | 判分链**指着**响应 JSON 的具体部位走：当前条件把被查的那一行点亮，第 6 条改为高亮右侧证据块；否决路径停在 branch 行并标出「预期 abstained」 | 「判分在查什么」从文字变成指针；矩阵缩到侧栏 |
| T5 验证与证据 | 6×5 的术语矩阵（11 个内部术语，图上从未定义） | **把块改坏的实测**：五种改法 × 两列「逐块 hash 变没变 / 两块整串变没变」 | 三行出现「hash 没变 / 整串变了」= 指纹盲区。两列状态由导出脚本把块真改一遍后实算，不是断言；原矩阵降到折叠层 |

**T5 的实算值得单记一笔**：「content_sha256 有盲区」原本只能作为断言写在页面上。它其实是可以算的——
把块按五种方式改坏，各自重算逐块 hash 与两块整串，就能看出哪些改法让 hash 全绿而整串已经不同。
导出脚本现在这样做，并断言「必须存在至少一种 hash 盲区」，否则这一块的结论不成立、产物不生成。
结论从此由计算保证，而不是由文案声明。

**度量（返工后，六块）**：桌面默认态 0.91–1.32 屏（全部在 1.5 屏护栏内），手机 1.86–2.70 屏，
均无横向溢出，各有独立结论锚。T5 手机从 3.79 屏降到 1.97 屏（矩阵移入折叠层所致）。
...
| ③ 对象与数据形状 | 7 文件 × (bytes、chars、行数、sha256 / git blob 前 8 位、三项比对状态)；一次校验 3 步；1 个 token 估算值 + 分类 `estimate` + tokenizer 版本。逐文件状态由导出脚本每次构建复算（§7.4 修正） |
...
| ⑨ 动效策略 | ①语义过程动效：三帧 = 读取快照文件 → 复算三项指纹 → 与 manifest 逐文件比对；每帧只引入一步，且三帧对应的状态各不相同（未读取 / 已复算未比对 / 已比对一致），不是同一张图换文字。可暂停、单步、重放；`prefers-reduced-motion` 下停在末帧 |
| ⑩ 验收证据 | 验收句：完整性校验的比对粒度是什么？断言：三帧各自的状态格取值不同（21 格 read → 21 格 computed → 21 格 match）；每份文档都比对 bytes/sha256/gitBlob 三项；最长条 = 最大文件；三单位排名不一致的文档被标出；两视口截图 |
...
### 7.4 corpus 完整性校验：从一次性记录升级为每次构建复算

**原判断**：D1 笔记记录「manifest 汇总与当前内容的逐文件字节/SHA-256/Git blob 校验均通过」（2026-09-07），
但 `week13-rag/` 内没有可重跑的校验命令（`scripts/` 只有 `w13rag.sh` 与 `inspect-block.sh`，`cli.py` 不复算
bytes 或 sha256）。因此第一稿判定 T1 只能按「一次性记录」呈现，不做逐文件状态格。

**修正（实现期，2026-09-09）**：manifest 记录的三项——`bytes`、`sha256`、`gitBlob`——**全部可以从文件本体
复算**（git blob = `sha1("blob <字节数>\0" + 内容)`，与 `git hash-object` 同一算法）。展示资产侧的导出脚本
`export-w13-rag-data.mjs` 本来就要读这 7 份文档，顺手做一次逐文件逐项比对既不新增读取，也不写入
`week13-rag/`。它已实现：任一项不符即抛错，产物不生成。

**因此 T1 的等级从「一次性记录」升为「每次构建复算」**，③⑤⑨⑩ 四列相应恢复逐文件状态格与实时通过态。
这与 §3.3「实现只新增展示资产文件」不冲突——新增的脚本在 `week8-fullstack/src/frontend/scripts/`，
`week13-rag/` 仍是只读。D1 的 2026-09-07 记录不被替代，它仍是当时的独立证据。
...
| 11 | corpus 完整性校验是否补可复算证据 | 补，但落在展示资产侧：由 `export-w13-rag-data.mjs` 每次构建复算 bytes / sha256 / git blob 并逐文件比对 | 三项都可从文件本体算出；导出脚本本来就要读这 7 份文档，不新增读取也不写 `week13-rag/`。详见 §7.4 |
| 12 | T2 的分布图放默认态还是折叠层 | 折叠层（与第 6 项一致） | 默认态只留扫描舞台与主结论；四张分布图进折叠，实测默认态 0.94 屏 |
...
五块 T1–T5 全部实现（2026-09-09，实现方模式）。以下为实现方自测证据；本人验收（视觉规范 §6 的人工判断）
待做，本节「人工视觉验收」一栏是实现方按同一清单的自查，不替代本人结论。

| 项 | 内容 |
|---|---|
| 实现日期 | 2026-09-09（T5 + T3 先交付，同日续做 T2 → T1 → T4，顺序按 §14 第 7 项裁决） |
| 新增 / 修改文件 | 新增 `scripts/export-w13-rag-data.mjs`（数据导出，只读 `week13-rag/`）、`src/w13RagData.ts`（生成物）、`src/w13RagTopics.ts`、`src/W13RagBoard.tsx`；修改 `aiEngineerTopics.ts`（`AeGroup` / `AeEvidence` 各加一项、`AE_TOPICS` 追加五块）、`AiEngineerBoard.tsx`（分发五个 kind、W13 块不显示 W12 概念入口、事实等级说明）、`styles.css`（`w13-*`，含 720px 与 reduced-motion 块）、`scripts/verify-w9-board.mjs`（§F 共 88 条断言；两条 W12 断言的组数与标签数随之更新） |
| 构建输出 | `yarn typecheck` 通过；`yarn build:showcase` 通过；`yarn verify:board` 1,633 项通过、1 项失败（见下行）；`yarn audit:visual` 采样 180 个视口专题状态 |
| verify 的一处存量波动 | W12 笔记流的既有断言「笔记间真实链接保持原专题返回上下文」在多次运行中时通过时失败（期望 `section=2`，实得 `section=12.6`）。在 `origin/main` 的独立 worktree 上同样失败（同一断言、同一取值），属存量波动；本次未改动笔记流 |
| 度量 · 桌面 1440×1000 | 五块默认态屏数：rag-composition 0.92、rag-scan 0.94、rag-freeze 0.97、rag-eval 1.01、rag-coverage 1.26；全部 ≤ 1.5 屏护栏，无横向溢出，各有独立结论锚 |
| 度量 · 手机 390×844 | rag-scan 1.89、rag-composition 2.27、rag-eval 2.27、rag-freeze 3.02、rag-coverage 3.79 屏；均无横向溢出，触控目标 ≥ 24px |
| 度量 · 主路径字数 | 常驻正文（10 秒结论 + 边界 + 记忆点）中文字数：rag-composition 119、rag-coverage 119，其余三块同量级；均在 220 字目标内 |
| 结构断言 · T5 | `content_sha256` 行只有 `model_content` 一格有值；职责边界列只落在无自动化行；6×5 = 30 格且空格渲染「管不到」；9 条测试各有清单项、映射无差集、点击后至少高亮一格；三条已实测事实为真 |
| 结构断言 · T3 | 七行四合计三增减且恰一段下降；三条恒等式在页面数值上闭合；下降段起点 = 核心正文终点、终点 = 原文终点；两段上升各起于上一合计终点；合计条共用左基线；块内无 token 单位的量；四帧只增不减；reduced-motion 下挂载即终态 |
| 结构断言 · T2 | 16 行全渲染、初始只扫到第 1 行且无块边界；扫完落下 6 个块边界且都是单行核心 span；末态标题栈两层且用缩进编码；标题 / thematic break / 表头行都不落块边界；thematic break 行在版面上是上下虚线的不可跨越边界；折叠层四张分布图，块类型计数之和 === 572 |
| 结构断言 · T1 | 三帧状态各不相同：第 1 步 21 格未复算且不显示任何指纹值 → 第 2 步 21 格已复算未比对 → 第 3 步 21 格与 manifest 一致、0 格不符；每份文档都比对 bytes/sha256/gitBlob 三项；最长条 = 最大文件；三单位排名不一致的 2 份文档被标出 |
| 结构断言 · T4 | answered 链 8 条全绿无停止标记；否决路径只推进 1 帧到底，停止标记落在第 1 步分支判定、其余 4 条标为不再推进；否决结论明说不被其它题分数抵消；矩阵五行且格内编码预期分支（四类 answered、无答案类 abstained）；受保护 split 整列上锁只出题数；metric 表只有契约四列、六行阈值格只有契约阈值或「无阈值（诊断）」两种取值 |
| 结构断言 · 受保护内容 | 分两层：① 导出脚本源码不出现受保护 split 的目录名（比在产物里做关键词匹配更贴近「没读过那个目录」这件事）；② 产物中不含该 split 的 item id 与 split 标记。字段名（`expected_rule_conclusion`、`corpus_absence`）不作探针——它们在公开的判分契约里被引用，用作探针只会误报 |
| 人工视觉验收（实现方自查） | 遮住标题与结论段：T3 从瀑布能答「89,854 里只有 32,171 来自核心正文」；T5 从矩阵能答「hash 行只有一格」；T2 从阶梯与刻度能答「标题不成块、`---` 不跨越」；T1 从三格指纹能答「比对是逐文件逐项的」；T4 从停止标记位置能答「否决在第一步」——五块均通过。10 秒结论均 ≤ 40 字；首屏均先出视觉舞台；记忆点均来自技术关系；无装饰图标。**本人验收待做** |
| 截图 | 五块的桌面 / 手机截图由 `CHROMIUM_PATH=… SHOWCASE_AUDIT_SCREENSHOTS=1 SHOWCASE_AUDIT_TOPICS=… yarn audit:visual` 生成到默认输出目录（按仓库惯例不入库）；T3 / T5 另拍过深色模式截图，填充图案、虚线下降段与矩阵空格在深色下均可读 |
| 边界 | 事实等级全部为「产物复算」；页面不出现模型运行结果，也不出现任何 metric 实测值；颜色第二编码：T3 四层用实心 / 斜纹 / 空心描边 / 点线，T5 用实心与空心方块加文字，T4 预期分支用描边形态（answered 单线 / abstained 双线）区分；判分契约原文里的反引号渲染为 `<code>`，正文不留 Markdown 残留 |
| `week13-rag/` git status | 无改动（本次只新增 / 修改 `week8-fullstack/src/frontend/` 下的展示资产与本方案文档） |
| 遗留锦上添花项与代价 | ① T5 手机端 3.79 屏、T1 3.02 屏，需翻屏复习；桌面不受影响。② 矩阵格与状态格高 38–40px，低于视觉审计采样器的 44px 触控参考（verify 的 24px 断言通过）；不改的代价是手机上误触相邻格的概率略高。③ T3 瀑布与 T2 分布图只提供数值直标与表格视图，没有 hover tooltip；不改的代价是无法悬停看分项。④ T1 第 1 帧的状态格视觉上接近空输入框；不改的代价是首次进入需读一行解说才知道那是「尚未读取」 |
