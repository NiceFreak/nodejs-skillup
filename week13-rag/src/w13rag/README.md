...
>
> 契约依据：语料冻结 `corpus/rules-c0a4b85/manifest.json`（D1）；eval `w13-eval-v1`（D2）；
> serialization 单一规范 `notes/day3-freeze-serialization-contract.md` §6.2.0（D3）；
> source block / registry 语义 `notes/day2-freeze-eval-contract.md` §6.1（D2）。
> 判据 #1–#7 逐字：`notes/serialization-criteria-confirm-checklist.md`（已确认）。

## 1. 六个模块一览

| 模块 | 职责 | 主要输入 → 输出 | 被谁依赖 |
|---|---|---|---|
| `__init__.py` | 包标记 + 一句说明 | — | — |
| `source.py` | 读取快照文档字节，建立行模型 | `snapshotPath` → `SourceDoc` | serialize / parser / registry / cli |
| `serialize.py` | 行规范化、`model_content`、hash、wrapper、Evidence Context 纯函数 | `SourceDoc` + 行号 → `str` / hash | registry |
| `parser.py` | 确定性 Markdown 块切分 | `SourceDoc` → `list[BlockInfo]` + `code_lines` | registry / cli |
| `registry.py` | 把 `BlockInfo` 变 registry entry；排序、查重、前置条件；拼整串 | `(doc, blocks, code_lines)` → entries / 整串 | cli |
| `cli.py` | 命令行入口 `build` / `verify`；逐文档审计；写证据文件 | `snapshot_root` → 证据文件 | （最外层） |

依赖方向（只有 `source.py` 不依赖包内其它模块）：

```text
source.py  <── serialize.py
   ^              ^
   └──── parser.py┘
   ^        ^
   └── registry.py ──> cli.py
   （registry 同时依赖 parser 的 BlockInfo 与 serialize 的纯函数）
```
...
- **行号全部 1-based、含两端**（D2 source identifier）；0-based 只在读取层内部出现，`line k = lines[k-1]`。
...
| 函数 | 作用 |
|---|---|
| `normalize_plain_line(raw)` | 普通行规范化：删行尾空白；≥2 空格（hard break）保留为恰 2；行首缩进原样 |
...
### parser.py — 确定性 Markdown 切分（唯一「有判断逻辑」的模块）
...
- `parse_blocks(doc)`：主函数。维护 `head_stack`（当前小节标题链），逐行分派到
  代码(可并入紧邻前置段)/标题/`---`/`>`引文(递归)/表格/列表/段落。
- `_mini_scan_inner` + `_parse_quote_region`：blockquote 递归——剥 `>` 后当小文档扫，
  再把结果映射回原文行号，产出 `quote_*` 类型、`quote=True`（保留 `>` 逐字）。
...
- `cmd_verify`：内存重建，与 on-disk 产物及（可选）冻结基准文件比对；不一致 FAIL 非零退出。
- `_line_class_is_structural`：审计里判定结构性行（与 parser 的分类保持一致）。


## 4. Python → TypeScript 概念映射
...
| `@dataclass(frozen=True)` | `interface` 全字段 `readonly` + `Object.freeze` | 不可变性要显式做 |
...
| `hashlib.sha256(x).hexdigest()` | `crypto.createHash('sha256').update(x).digest('hex')` | `update` 需 Buffer 保证 utf8 一致 |
...
| 1-based 行号 | 无内建支持，靠命名与注释约定 | 全代码库统一 1-based、含两端 |
| `int \| None` | `number \| null` | — |

### 4.2 各模块的 TS 关键形状（签名级示意，非完整实现）
...
// 正则直接复用：_MARK_RE / _HEADING_RE / _HR_RE / _DELIM_RE / _ROWLIKE_RE
```
...
| 文件 | 作用 |
|---|---|
| `tests/conftest.py` | 把 `src` 加入 `sys.path`（week13 未安装为包，pytest 靠它 import） |
| `tests/test_fixture_serialization.py` | fixture A/B/C 的 `model_content` / hash / wrapper / 整串字节回归 |
| `tests/test_registry_real.py` | 真实语料不变式：无遗漏/重复、双跑一致、hash 复算、wrapper 前置条件 |
| `scripts/README.md` | 脚本导读：什么时候跑哪条、输出字段含义、退出码判了什么、失败先查哪里 |
| `scripts/w13rag.sh` | CLI：`test` / `build` / `check` / `verify` |
| `scripts/inspect-block.sh` | 打印单个 registry entry（review 用） |
| `evidence/serialization/` | 产物：registry JSON、整串 txt、sha256、criteria-report、`frozen-*.sha256` 冻结基准 |

运行（均在 `week13-rag/` 目录下）：
...
FROZEN_SHA256=evidence/serialization/frozen-rules-c0a4b85.sha256 \
  ./scripts/w13rag.sh verify                      # 内存重跑 vs on-disk vs 冻结基准
...
### 4.3 翻译到 TS 时要额外小心的事（Python 不会提醒你）

1. **确定性 / 键序**：Python dict 保持插入序且我们从不依赖遍历顺序做哈希之外的事；TS 若用
   `JSON.stringify` 把对象当输入，键插入顺序必须固定，否则「同一内容」会因键序不同产生不同串。
2. **正则语义**：Python `re.match` 从头匹配、无 `g` 状态；JS `RegExp.exec` 若带 `g` 会共享
   `lastIndex`，容易出隐式 bug——用无 `g` 正则或每次新建。
3. **1-based 行号**：TS 数组天然 0-based，`lines[lineNo-1]` 这类换算点必须集中在读取层，
   不要在 parser/registry 里散落 ±1，否则和现在 Python 里 `a+1 / e` 的换算一一对应易错。
4. **不可变**：`frozen` 语义不会自动存在，需 `readonly` + `Object.freeze`（或约定不 mutate）。
