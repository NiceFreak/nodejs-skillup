...
- [ ] 第一入口：关闭报告、D3/D4 笔记与展板，完成 Bub 四个独立问题口述；AI 逐题验收，不提前提示。
- [ ] 本人在 45 分钟内独立诊断未提前见过的 Python 异步/资源管理代码并冻结答案；冻结前不得启动
  Codex/Cline。
- [ ] 从 D4 三类预测偏差中任选一类，复述原判断、实际现象、修正条件与验证证据。
- [ ] 本人答案冻结后，Codex/Cline 对同一输入执行只读 review，合计不超过 60-90 分钟。
- [ ] 类 2 最小样本第一档完整再重建；通过时核对两项掌握证据后关闭债务，卡档则如实记录。
- [ ] 接入 pytest-cov；`src` 增量迁移代码行覆盖率 >= 90%，pytest、mypy 与 smoke 全绿。
- [ ] 完成 Bub 阅读报告：复核事实/推断/待验证，回填 C1 等价结构实验与 D4 cancellation 的适用边界。
- [ ] 盘点 tracked Markdown 规模与排除类别，按 D2 冻结的唯一验收句逐项判定五项交付物。
- [ ] 更新 `LEARNING-STATE.md`，记录 W12 交付判定、独立掌握判定、W13 第一入口与 W14 Bub 重建日期。
