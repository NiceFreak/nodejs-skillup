...
单日计划见 [`day5-diagnosis-and-wrapup.md`](./day5-diagnosis-and-wrapup.md)（2026-09-04 建立）。9/3 学习效果
复盘指出独立诊断、脱离材料复述和综合迁移证据不足，因此 D5 不增加范围，将独立能力验收移到配置与报告
收尾之前；本周五项交付物和既有验收门槛不变。

- [x] 第一入口：关闭报告、D3/D4 笔记与展板，完成 Bub 四个独立问题口述；AI 逐题验收，不提前提示。
      —— Q1/Q3/Q4 首答通过；Q2 首答未通过（把 `_default_messages` fallback 当默认路径）→ L1 讲解 +
      本人读码修正重答通过。缺口与修正全程留痕（day5 §5.1）。
- [x] 本人在 45 分钟内独立诊断未提前见过的 Python 异步/资源管理代码并冻结答案；冻结前不得启动
      Codex/Cline。—— 题目 `experiments/d5_diagnosis_fetcher.py`（共享 client 在 fetch_one finally 被
      关闭）；is_closed 打点定位根因 + 修复假设 + 验证设计；AI 验收通过（day5 §5.2）。后续亲手运行
      修复验证（0.254s + 反证）闭环。
- [x] 从 D4 三类预测偏差中任选一类，复述原判断、实际现象、修正条件与验证证据。—— CPU 忙循环类，
      四项复述与 D4 记录对照通过（day5 §5.4）。
- [x] 本人答案冻结后，Codex/Cline 对同一输入执行只读 review，合计不超过 60-90 分钟。—— Codex 首试
      502 → W12 内重试成功（chatgpt 5.6）；Cline/Claude Code 完成；四方结论一致，裁决表见 day5 §5.3。
- [x] 类 2 最小样本第一档完整再重建；通过时核对两项掌握证据后关闭债务，卡档则如实记录。—— 完整
      第一档通过（连续第 2 次）+ 证据 A/B 核对通过 → **DEBT.md 已还（2026-09-04）**（day5 §5.5）。
- [x] 接入 pytest-cov；`src` 增量迁移代码行覆盖率 >= 90%，pytest、mypy 与 smoke 全绿。—— 30 passed /
      97.89%（口径内 clients/config/models）/ mypy Success / smoke exit 0（day5 §5.6）。
- [x] 完成 Bub 阅读报告：复核事实/推断/待验证，回填 C1 等价结构实验与 D4 cancellation 的适用边界。——
...
**D5 后遗留项收口（2026-09-05）**：B3 展板实现与机器/截图验证完成，仓库主人的 B3 图示人工验收
当时仍待完成；vendor 快照按成本取舍记录为未做；论断 10 的真实 TCP/TLS 实验记录为未做。详见 day5 §5.9 与
...
- 2026-09-04：AI 以实现方模式（白名单学习计划）建立 D5 单日计划
  [`day5-diagnosis-and-wrapup.md`](./day5-diagnosis-and-wrapup.md)，并根据 9/3 学习效果复盘把独立能力验收
  移到 pytest-cov、报告和周间接口之前。未预填诊断题、重建题、本人答案或黑名单核心设计，不新增债务。
