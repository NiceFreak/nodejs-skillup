...
JWT 签发阶段已通过验收。
两份 Postman 资产已经同步：
登录成功断言改为 body.payload.user
校验 v2 的 code/message/payload
校验 accessToken 存在
自动保存为 collection variable accessToken
JSON collection 已通过格式校验
[LEARNING-STATE.md] 也已更新为“签发已验收，下一步设计验证中间件”。
进入下一步前，请在 [day3.md] 继续回答验证中间件契约：

1. token 从请求的哪个位置读取？接受什么精确格式？对于 `Bearer` 的大小写采用标准兼容还是项目严格契约？
2. 验证成功后分开决定：保留 payload 中的哪些身份声明？把认证结果挂到 `req` 的哪个自定义字段？
3. 只讨论 Authorization/JWT 认证凭据：缺少 Authorization、scheme/空 token 等格式错误、JWT 结构错误、签名无效、token 过期分别应该返回什么 HTTP 状态？错误信息是否需要区分？
4. 中间件是否需要查询用户数据库？当前阶段为什么选择查询或不查询？这个结论是否适用于所有系统？
5. 认证、通用路由级授权和依赖资源状态的业务授权分别解决什么问题？哪些适合认证中间件、独立 RBAC 中间件和 Service？Controller 在其中承担什么职责？
6. 中间件成功后为什么必须调用 next()，失败时为什么不能调用？

回答:
1. token 从请求的 request headers 中读取, Express 中对应 req.headers.authorization, 接受 Authorization: Bearer <token> 的精确格式. Bearer 大写开头, 后接空格, 之后是 token, 我们将这个定位契约.
2. 这个问题不太精确, 把多层语义混在了一起. "什么身份信息" 是一个问题, "放到 req 的哪个字段" 是另一个问题. 对于前者, 指的是从 jwt 的 payload 中解析出所需的最小身份标识, 在当前契约中就是 sub. iat和 exp对于业务来说是不需要用到的. 对于后一个问题, req 贯穿了自上到下和自下到上的两个阶段, 正常来说应该由中间件自定义一个字段来存放, 比如 req.auth. 
req.auth = {
  sub: 已验证的用户 ID
}
3. 所有 token 相关的格式错误是, 或者 没有 token、签名被篡改, token 过期 都是 401
缺失、格式错误、结构错误、签名无效、过期
→ 全部 401
→ 对客户端统一使用同一条无效凭证文案
→ 服务端日志可以记录具体原因
4. 不需要, 当前阶段查询数据库不应该放在中间件, 我们有 sub 了, 当前阶段的中间件, 保持无状态是最好的实践, 我们的契约选择不查
5. 认证中间件回答“你是谁”，独立的授权中间件回答“你能否访问”。权限判断不一定直接放业务 service(读取 http 输入时由 controller 调用)，也可以是后续的 RBAC 中间件, 按实际分层为准
6. 不调用 next(), Express 不会往下一步流转, 不能进到下一个中间件,controller,失败时应该进全局错误处理器, next(error)

认证中间件
→ 验证 token，回答“你是谁”

通用授权中间件
→ 判断 admin/member 等路由级权限
→ 回答“这个角色能否进入该接口”

Controller
→ 读取 HTTP 输入，调用 Service，组织 HTTP 响应

Service
→ 根据资源和业务规则进行授权
→ 例如“当前用户是否拥有这笔订单”“订单当前状态是否允许修改”
