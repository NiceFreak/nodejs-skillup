...
### P2-4 · **九块板**按加严后的验收判据复核 —— 规模：**大**（2026-08-20 范围扩大，**档位待本人确认**）
...
核对后成立——8/18 的落地范围只覆盖 W10（返工）+ W9（延后），
**其余七个 tab 的板从未进入过新判据的范围**：`W3` / `W5` / `W6` / `W6Day4` / `Auth` / `Interview` / `Dashboard`
的组件最后一次改动**都停在 2026-08-12**，是在旧规则下建成的；
8/18 的「存量清扫」(`d24038b`) 只推了 `styles.css` 的排版结论，**没碰内容形态**。
故复核对象 = **九块板**（上述七块 + W9 + W10），证据见 roadmap 第八轮「落地范围的补正」。

**规模从「中」升到「大」**，因此拆两步，**不许跳过第一步直接开改**：
1. **定性**（排在 **W10 D5 块 G**，25 min）：抽样 3 块跑「四条肯定面 + 遮标题仪式」，点出欠着的块、估出总规模。
2. **返工**（独立排期）：按定性结论决定改哪几块，不必九块全改。

**档位待确认**：原 P2 的理由是「W9 板刚核过、成本高于收益」。范围扩到九块后成本更高，
但**它同时是唯一对外可展示的资产**（8081 带门禁的学习展板），
究竟仍按 P2 排还是升档，等 D5 块 G 的定性结论出来再定——**在此之前不动档位**。
