...
1. **阶段一 · 只判定不改**：八块 × 每块一个 15–20 分钟单元，记「欠什么、值不值得改」。**2026-08-20 已完成八块**。
2. **阶段二 · 按判定结果返工**：改哪几块、改到什么程度，一次性决定；大概率不必八块全改。**尚未开始**。

**阶段一结论（2026-08-20，判定记录与依据见核查线 §5）**：**八块里两块达标、五块小欠、一块大欠**。
第 4 块（Node.js 运行时）与第 8 块（可观测性）四条肯定面全不破，不必改；
第 2 / 3 / 5 / 6 块各欠一处，各 1 个单元；第 1 块只改下半，2 个单元；
第 7 块（部署上线，十三块 · 30872 中文字 · 39 个 `h3`）欠板级锚，是唯一的大头，估 3–4 个单元。
**总规模 8–10 个单元**（约 2.5–3.5 小时，可散在空档里）。
另判定「面试准备」板**不纳入**返工范围（`iv-matrix` 已承载其验收句）。

**真正的大头不是逐块返工，而是两条共性欠口**：① **互斥切换**——第 1/3/4/5/6 块都用 `nav` / `tablist`
把同类对象切开，读者被迫在脑子里建表；② **量的差距只写成数字**——第 3、6 块（以及共用同一份
`evidenceSets.ts` 的面试板）都是「差距本身是结论」，而 `HBarChart` 已存在、W9 已在用，属复用不属新造。
这两条一次做完，覆盖大半欠口。

**档位定了：仍按 P2，不升档。** 上一版留的问题是「范围扩大后成本更高，是否升档」——
判定结果推翻了这个预期：八块里两块本来就达标，欠口高度同型，
**扩大范围反而让规模变小了**，因为「八块都欠着」是推断，逐块看过之后它不成立。
（这一条结论是 AI 预判，「遮住 `h3` 能否答出验收句」的强制力等级仍是「人」；本人复核后若结论不同，档位随之重定。）
