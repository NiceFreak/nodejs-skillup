...
| 5 | 监控表态逐项核对 | 每类注入前**预测四项 check 各自绿还是红**，注入后核实测——**预测全绿的那一类就是覆盖盲区，要写下来** |
| 6 | 基线全恢复 | 收工时三层基线（五面 + `/health` + 三服务 + 四 timer）全绿，且**演练残留清零**（§2.4 L1 清单） |

> 判据 5 是 D3 → D4 的真正分工：D3 证明了「检查判据能红」（假输入），
...
| ③ | **验收** | `systemctl list-timers check-cert.timer` 那一行的 **NEXT 与 LAST 相隔 6 小时**（同样的错误再犯一次也会被这个动作抓住） |
...
> 抄一个过期的数是本周第六条「检查全过但语义没生效」的现成候选。
>
...
### P1（延续验证还是重复验证）证书那一类在 D3 已经被用掉了，D4 还剩什么可验证？
...
| b. 换一个不碰现网证书的延续验证形态 | 需要本人先答：正式路径下还有什么是 D3 没验过的？（例如**读不到**时脚本是报红还是报错退出——这是两种完全不同的失效） |
...
>
> **P1 定案 = 选 a + 块 B 正式路径验证通过（§6.4）+ runbook 首查用 check-cert。**
...
1. **首查项**（一条具体命令，不是「看日志」）；
2. **为什么先看它**（它的两种结果各自把范围收窄到什么，即 D1 §2.4「排障即二分」）；
...
> **答（2026-08-20 块 C 定案）：**
>
> **类 1 与类 2 首查项共用 `curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:3000/health`，一步区分两类：**
...
### P3（监控覆盖核对）每一类注入后，四项 check 分别是绿还是红？
...
> 补盲候选（已评估代价）：
> - **本地 Nginx 后端健康检查**（curl 每个 `proxy_pass` 后端，失败告警）——推荐；代价：维护后端列表 + 每次 N 次 curl（<100ms）。**边界（review ②）：抓得住「目标死/无服务」，抓不住「目标活着但指错端口」（curl 返回 200、业务 502）；后者兜底证据是 error.log `connect() failed` 里的端口号。**
...
| 块 | 时长 | 内容 | 产出 / 收工判据 |
|---|---|---|---|
| A | 25 min | **前置修正**（§2.0）：改 `check-cert.timer` → `daemon-reload` → restart → `list-timers` 看 NEXT/LAST 间隔核对 → 同步仓库副本 → 展板那一格翻档 | NEXT 与 LAST 相隔 **6 小时**（不是「文件里写对了」）；仓库副本 `diff` 为空 |
...
**块 A 和块 H 一步都不许砍**：A 是今天全部证据的信任基础，H 是这台唯一生产机今晚能不能无人值守运行的判据。
...
systemctl list-timers check-cert.timer    # 验收看这里的 NEXT 与 LAST 间隔
```
...
- 首查项 + 为什么先看它：`df -h /`。结果分成两支：可用 < **3.5G**（执行期修正，见 §2.3 注）→ 触止步，立即清理（MongoDB 同分区）；≥ 3.5G → 磁盘不是根因，转 `free -m` / `journalctl --disk-usage`。
...
**③ 定位**（我实际先看的是什么、看到什么、它把范围收窄到了什么、下一步走哪边）
- 注入后首查 `df -h /` + `df -B1 /`：avail = 3.84 GiB，落在计划区间 → 磁盘占用符合预期，占位文件是唯一嫌疑（`ls -lh /tmp/disk-fill.bin` 确认）。
- 15:00:01 timer 触发后查 `journalctl -u check-disk.service`：**第一次看到 OK 而非 FAIL** → 把范围收窄到「判据口径」而非「注入量/止步」——avail 够低（3.84 GiB < 4 GiB 字节级）但脚本判绿。
...
- 类 3 验收判据逐条核对：类数✅ / 有证据✅（注入+15:00 journald OK 行）✅ / 定位顺序✅ / 事实分层✅ / 监控表态预测 disk🔴→实测🟢 偏差已归因✅——「该红不红」正是预测偏差的最大价值点。
...
```text
443root 502       # ✅ 命中预测
health  200       # ✅ 命中预测（首查区分：Node 正常 → Nginx 层）
...
```
→ **四项 check 全绿（Predict 全中）= 盲区实锤**：反代配置错（443root 502）在注入态四项全绿，故障只能靠人工访问/error.log 发现。
...
- 不建 Prometheus / Grafana（stretch，受常驻内存上限约束）；不接 webhook 告警通道（D1 Q11 已降 stretch）。
...
> **状态（2026-08-21 回填）**：十一项全部完成。依据 —— §6.1/6.2/6.3 三类五段式记录各含命令输出、§6.4 写明证书类为什么不做；§10.4 残留逐条核零且三层基线 15:30 全绿；§10.1 timer 修正已用 `systemd-analyze` 间隔核过、仓库副本 diff 为空；`week10-plan.md` §4 的 D4 已勾选并回填结果一句话；`day4-english-speaking.md` 已入库。

- [x] §6 四份（或 3 份 + 一份「为什么不做」）记录填满，每份**至少一条命令输出** → 3 份五段式 + §6.4 证书类「为什么不做」
- [x] §1 的六条验收判据逐条核对，缺哪条写清楚缺在哪 → 对照见 §10.5
- [x] §2.4 残留清单逐条核零（`diff` 为空 / `ls` 不存在 / `pgrep` 无匹配） → §10.4
- [x] 三层基线最终回归全绿，输出贴进 §10 → 15:30 五面 200 + health 200 + 7 active
- [x] §2.0 的 timer 修正已用相邻触发间隔核对（6 小时），仓库副本已同步（diff 为空） → §10.1
- [x] `week10-plan.md` §4 的 D4 勾选 + 结果一句话
- [x] `LEARNING-STATE.md`：当前 Day、最近完成、验收命令或证据、下一步（指向 D5）
- [x] 必要时 `DEBT.md`（今日预期不触发：白名单语法 + 黑名单止步 L2）→ 实际未触发，`DEBT.md` 无 W10 条目
- [x] `day4-english-speaking.md`（按 `DAILY-SPEAKING-PROTOCOL.md`，**主题取自当天真实发生的事**，不写计划里没做成的）
- [x] 展板 ⑤「演练分档」块：D4 之后那四类的「首个症状」才从预测变实测——本块能否落地由本人决定，不阻断收口 → 当天落地，`verify:board` 421/421
- [x] commit
...
**已完成（上午）**：`check-cert.timer` 的 `OnCalendar` 修正为 `0/6:00:00`；`systemd-analyze calendar '0/6:00:00' --iterations=3` 输出相邻间隔 6 小时（12:00→18:00→次日 00:00）核对通过；仓库副本已同步（diff 为空）；展板 ⑦ 频率表翻档；`yarn verify:board` 396/396 全过。
...
- check-cert 11:43:55 OK（**无 CERT_OVERRIDE —— 正式路径读得到、读得对，P1 选 a 验收达成**，输出见 §6.4）
...
- 同时把 D3 §9.5 的 timer 频率修正落成 §2.0 的开工前置（含**验收方式必须是 `list-timers` 的间隔、不是读文件**），
  这是 D3 §9.5 处置清单的原文搬运，非新决策。
- 2026-08-20（D4 当日，块 C）：P1–P5 本人全部作答，AI 逐题 review——
  - **P1 选 a**（证书类由 D3 覆盖，D4 只做 3 类）+ 块 B 正式路径验证通过 + runbook 首查用 check-cert；
  - **P2 首查**：类 1/类 2 共用 `/health` 一步区分反代层与应用层（类 1 预期 200、类 2 预期非 200），类 3 用 `df -h /`；
