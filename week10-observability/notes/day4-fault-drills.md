...
| # | 判据 | 怎么算过 |
|---|---|---|
| 1 | 类数达标 | 完整走完 **≥ 3 类**（契约四类为目标，3 类是下限，见 §2.5 止步①） |
...
**明确不在清单里**（动了就是超纲）：不改 `app.js` / `server.js` 一行；不改四个 check 脚本的任何阈值常量（那是 D3 的弄红手法，今天要的是**真条件**）；不动 `journald.conf`；不碰 8081 展板产物；不碰 8080 下线。

### 2.2 执行顺序（本人拍板，见 §3 P4）

```text
块 A 前置修正（§2.0）→ 块 B 基线复位 → 块 C 答 §3 五题
→ 类 ? → 类 ? → 类 ? → 类 ?   ← 顺序由 P4 决定，不临场排
...
| 4 | 证书（形态待 P1） | B | 待填 | 待填 | 脚本报错/误报；脚本被误改碰现网证书路径 | `rm -f /tmp/test.crt /tmp/test.key`；恢复脚本备份 | ☐ |
...
**残留清单（收工时逐条核零，块 H）**：
`/tmp/disk-fill.bin` · `/tmp/test.crt` `/tmp/test.key` · 抢占端口的 socat/nc 进程 · `shop-ssl` 与备份的 `diff` 为空 · 四个 check 脚本与 `.bak` 的 `diff` 为空（今天本来就不该改它们）· systemd unit 里没有临时加的 `Environment=` 行。
...
| D | 40 min | **第 1 类**（顺序按 P4）：注入 → 现象 → 定位 → 根因 → 修复 → 恢复基线 | §6 该类五段式填满 + **至少一条命令输出**；三层基线复绿 |
| E | 30 min | **第 2 类**：同上 | 同上 |
| F | 45 min | **第 3 类**：同上（若含「等真实触发」，等待期同步写 §6 前两类的根因段，不空转） | 同上 |
| G | 25 min | **第 4 类**：形态取决于 P1；若 P1 选 a（已被 D3 覆盖），本块改为把这个结论写进 §6 与 runbook 输入 | 同上，或有一段写明「为什么不做」的结论 |
...
### 6.1 类 ?：（按 P4 的顺序填类型与档位）

**注入前预测（块 C 写，注入前不许改）**

- 首查项 + 为什么先看它：
- 第二查（首查返回预期 / 返回意外，分别往哪走）：
- 预测的首个症状：
- 四项 check 预测（app / mem / disk / cert）：
- 前置四件事四格已核：☐
...
### 6.4 类 ?：（同上模板；若 P1 选 a，本节改写为「为什么这一类由 D3 覆盖、runbook 里那一条的凭据是什么」）
...
- 截至起草时**未触发 `DEBT.md` 记账**（白名单语法 + L1/L2 出题，黑名单零实现）。
