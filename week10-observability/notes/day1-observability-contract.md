...
- [x] 「监控常驻内存上限 ≤ N MB」与「上 Prometheus + Grafana」——**先量再装，不是先装再量**（Q7 × Q8）。**Q7 主线 (a) 脚本、stretch 上限 ≤80MB 超了卸载；Q8 available 基线 1304MB 已量。**
...
**Q7（监控档位与常驻内存上限）**：主线做哪一档——(a) 检查脚本 + `systemd timer` + 退出码；
(b) 只加 node_exporter 留数据；(c) 完整 Prometheus 栈？
必答追问：**stretch 的常驻内存上限定为多少 MB**，超了怎么办（卸载？还是接受？）——
这个数要能对上块 C 采到的 `free -m`（§2.4.5：`available` 才是离 OOM 的距离）。

> 答：主线选 **(a) 检查脚本 + systemd timer + 退出码**，不选常驻栈。三把尺子：本周验收只消费「能跑、能红、能绿」，不消费图表和趋势；swap=0 + 可用 1304 MB，常驻监控栈会挤占演练余量，脚本常驻≈0 最安全；stretch 留 (b)(c) 但设常驻内存上限 **常驻新增 ≤ 80 MB**（约当前可用的 6%），超过即卸载回滚 (a)——超限说明监控栈成本大于收益，止损。
...
## 5. 今日交付物（§5.1 / §5.3 / §5.4 / §5.5 已于 2026-08-17 填写完成）
...
> **实际结果（2026-08-21 回填）**：
> - 第 4 类**未在 D4 执行**——同一注入手法已被 D3 §9.2 ⑤ 用掉，D4 §3 P1 拍板选 a（由 D3 覆盖），理由与正式路径验证见 `day4-fault-drills.md` §6.4。D4 实际做的是第 1–3 类。
> - 第 3 类的 `26.5G` 是起草期估算值，D4 实测证明它已漂移：注入量必须按当天 `df` 基线重算（D4 §5.3 写明「不照抄 26.5G」；D5 重算为 26.4G）。

### 5.5 只读基线（块 C 采集，2026-08-17 已填）
...
- 不装 node_exporter / Prometheus / Grafana（档位未定，常驻内存上限未定）。
