...
> 最后更新：2026-08-21（Asia/Shanghai，W10 全周收口完成，状态指向 W11 入口；展板核查线阶段二进行中）
...
- **独立线（继续排队，不与学习主线同步开展）**：**展板状态核查线**（[`showcase-audit-line.md`](week8-fullstack/notes/showcase-audit-line.md)）——对象**八块板**（Auth/OAuth2/W3/W5/W6/W6Day4/W9/W10）。**阶段一 8/8 判完**（两维：视觉编码 + 阅读负担实测，45 个专题逐个量过）；**阶段二已做五轮**——常驻外壳（`W9`/`W10` −64%/−56%）、`W10` ⑤（−31%）、`W6Day4`（−42%）、`W9` `evidence`（−60%）、`W10` ⑦（−31%，8/21 解冻后补做）；每轮 `verify:board` 均 421/421、内容零删除、未部署。**第 4 步核实后判定不做**（「首屏 0% 编码」是选择器漏报，四个专题首屏其实都有位置编码，一行未改）——**阅读负担这一支五轮收口**。**视觉编码维两条共性欠口均已落地**：②（量只写成数字）→ `W3Board` + `W6Day4Board` 补行内长度编码；①（互斥切换）→ `charts.tsx` 加共用的 `StopMatrix`，`AuthBoard` + `W6Board` Day 3 把三条路径的停止点摆到同一根轴上。`OAuth2Panel` 经本人裁决**不改**。**全线只剩一项**：`W6Board` 常驻折叠的证据精度矩阵，做完即可按 §8 收口。另 `W10` ⑤ 第二轮待定、`W10 D5` 若新开 tab 由本人另开对话。档位、判定表、每轮实测与取舍全部在执行文档里，本文件只留这一行指针（该线 §7）。
