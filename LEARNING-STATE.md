...
D4 开工只需读 `week4-auth/notes/day4.md`：重建任务卡（含新通过标准）、D3 深夜投影讨论整理、`findOneAndUpdate` 预测实验、RBAC 主线清单都在该文件，当天过程也记在该文件。

1. D4 上午第一项：15–20 分钟自然月报表边界第一档重建；过程中 AI 不提示，只在结束后验收；通过标准=能当场推导（见 `AGENTS.md` 重建梯子）。
...
- **待固化的流程改进**：AI 引导式提问语义不精确，本周两次把多个设计点合并成一问（compare 与 JWT 签发同轮提问；D3 中间件契约第 2 问两问合一），造成流程混淆。改进约定已于 2026-07-15 固化进 `AGENTS.md`「引导式提问规范」一节：一问一个设计点、标注所属流程与阶段、回答者有权拆题。
- AI 尚未提供任何 W4 鉴权核心实现。
- 当前欠债状态：自然月报表边界第一档重建已通过，但还差至少一项掌握证据后才能标已还；注册竖切的文件职责与依赖方向、JWT 签发链路的配置校验与分层边界、最小 RBAC 授权链路仍需在 D5 上午或周验收前完成第一档重建。详见 `DEBT.md`。
