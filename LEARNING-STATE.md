...
- **独立线（不与学习主线同步开展，2026-08-20 起）**：**展板状态核查线**——阶段一 **8/8 已完成**（8/20 一次做完，只判定不改；结论：两块达标、五块小欠、一块大欠，总规模 8–10 单元，`BACKLOG.md` P2-4 档位维持 P2）。阶段二（改哪几块）尚未开始。执行文档 [`showcase-audit-line.md`](week8-fullstack/notes/showcase-audit-line.md)；排队不排期，只在当天主线达到止步条件且有 15–20 分钟空档时推进，零进展不算欠账。**不属于并行线**（8/17 拍板的并行线仍只有英语 + 项目叙述）。
...
- **展板状态核查线（白名单，8/20 晚）**：本人说「开启展板状态核查」→ 按核查线 §6 触发（主线已收口）；
  随后本人说「把这些全部查证完先」，§6「一次只做一块」当次作废，**阶段一八块一次判完**。
  AI 出判定与可复核依据（组件结构、`role="table"` / 轴 / 柱长的实际写法、`stopAt` 与指标取值、
  `<details>` 有没有 `open`、字段与字数统计），**全程零代码改动**。
  「遮住 `h3` 能否答出验收句」那道闸的强制力等级仍是「人」，故 §5 八行全标为 AI 预判、待本人复核。
