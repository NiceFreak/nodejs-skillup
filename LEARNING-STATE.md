...
- **独立线（继续排队，不与学习主线同步开展）**：**展板状态核查线**（[`showcase-audit-line.md`](week8-fullstack/notes/showcase-audit-line.md)）——对象**八块板**（Auth/OAuth2/W3/W5/W6/W6Day4/W9/W10）。**阶段一 8/8 判完**（两维：视觉编码 + 阅读负担实测，45 个专题逐个量过）；**阶段二四步已做三步半**——常驻外壳（`W9`/`W10` −64%/−56%）、`W10` ⑤（−31%）、`W6Day4`（−42%）、`W9` `evidence`（−60%）；每轮 `verify:board` 均 421/421、内容零删除、未部署。**剩 `W10` ⑦（4.35 屏，此前本人指示先等）与第 4 步四个小专题**。档位、判定表、每轮实测与取舍全部在执行文档里，本文件只留这一行指针（该线 §7）。
