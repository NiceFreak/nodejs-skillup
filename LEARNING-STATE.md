...
> 最后更新：2026-08-21（Asia/Shanghai，W10 全周收口完成，状态指向 W11 入口；展板核查线已按 §8 关闭，D5 内容已新建为第八块 ⑧ 上板）
...
- **独立线（继续排队，不与学习主线同步开展）**：**展板状态核查线**（[`showcase-audit-line.md`](week8-fullstack/notes/showcase-audit-line.md)）——对象**八块板**（Auth/OAuth2/W3/W5/W6/W6Day4/W9/W10）。**阶段一 8/8 判完**（两维：视觉编码 + 阅读负担实测，45 个专题逐个量过）；**阶段二已做五轮**——常驻外壳（`W9`/`W10` −64%/−56%）、`W10` ⑤（−31%）、`W6Day4`（−42%）、`W9` `evidence`（−60%）、`W10` ⑦（−31%，8/21 解冻后补做）；每轮 `verify:board` 均 421/421、内容零删除、未部署。**第 4 步核实后判定不做**（「首屏 0% 编码」是选择器漏报，四个专题首屏其实都有位置编码，一行未改）——**阅读负担这一支五轮收口**。**2026-08-21 已按 §8 关闭**：阶段一八块两维判完（阅读负担做成 45 个专题的实测），阶段二八轮——改六轮、判掉两轮、两处由本人裁决；内容零删除、`verify:board` 恒 421/421、**未部署**。收口记录见 roadmap 第九轮，`BACKLOG.md` P2-4 已标完成。**另立的两件**：① `W10` ⑤ **已按本人拍板在专题条上分成三页**（演练 / 检查表态与信号 / 盲区与收尾），一页 4.07 屏 → 三页 1.78 / 1.66 / 0.73 屏；**「本板共 7 块」口径不动**（那是建成了哪七块内容的历史事实），所以七块九个 tab；`verify:board` 421→**441**（新 tab 进入逐专题体检，覆盖面变大，判据无一放宽）；记录见 `week10-visualization-plan.md` §12.10。**两条复习闸门问句是新写的，待本人过目。** ② `W10 D5` **已新开 tab 并落地**（2026-08-21 另一次对话）：新建**第八块 ⑧「不看笔记还能不能走通」**，直接按新标准做成三页（第一刀 / 盲测两类 / 两条收尾的强度），实测 **1.01 / 0.98 / 1.03 屏**，是本板最轻的三页；**板尾口径翻成「共 8 块」**（这次是真多建成一块，与 ⑤ 的分页不同型），十二个 tab；`verify:board` **441 → 490/490**，**未部署**。执行记录与那处「新增 tab 把正文起点顶出第一屏、改三列修回」的回归见 `week10-visualization-plan.md` §12.11。**三条复习闸门问句是新写的，待本人过目。**档位、判定表、每轮实测与取舍全部在执行文档里，本文件只留这一行指针（该线 §7）。
