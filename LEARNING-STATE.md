...
- **独立线（继续排队，不与学习主线同步开展）**：**展板状态核查线**（[`showcase-audit-line.md`](week8-fullstack/notes/showcase-audit-line.md)）——对象**八块板**（Auth/OAuth2/W3/W5/W6/W6Day4/W9/W10）。**阶段一 8/8 判完**（两维：视觉编码 + 阅读负担实测，45 个专题逐个量过）；**阶段二已做五轮**——常驻外壳（`W9`/`W10` −64%/−56%）、`W10` ⑤（−31%）、`W6Day4`（−42%）、`W9` `evidence`（−60%）、`W10` ⑦（−31%，8/21 解冻后补做）；每轮 `verify:board` 均 421/421、内容零删除、未部署。**第 4 步核实后判定不做**（「首屏 0% 编码」是选择器漏报，四个专题首屏其实都有位置编码，一行未改）——**阅读负担这一支五轮收口**。**视觉编码维已开工**：共性欠口②（量的差距只写成数字）落地，`W3Board` + `W6Day4Board` 的指标表补上行内长度编码，一次覆盖 6 个单元里的 4 个（类别型的格子一条不画，W6 组整组不画）。**剩共性欠口①**（`Auth` + `W6` Day 3 互斥切换，2 单元，建议下一轮）；另 `W6Board` 证据矩阵常驻折叠、`OAuth2Panel` 凭据卡、`W10` ⑤ 第二轮待本人裁决。档位、判定表、每轮实测与取舍全部在执行文档里，本文件只留这一行指针（该线 §7）。
