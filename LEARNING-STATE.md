...
2. **展板现状（8/18 更新，`yarn verify:board` 349 项全过，未部署）**：
   - **W9 板十三块齐、已收口**，无待做块。仅 ⑭「产物与溯源」记 BACKLOG，非阻断。见 [`week9-visualization-plan.md`](./week9-deployment/notes/week9-visualization-plan.md) §12.19 与 §13。
   - **W10 板（tab `可观测性`，reviewOnly）落地 4/6 块**：⑥ 假生效、① 盲区、③ 日志旅程、② 字段销账已做；**④ 阈值尺、⑤ 演练分档待做**。按方案 §9 的顺序纪律「先做的会先说谎」：**④⑤ 押到 D3 / D4 之后**——④ 四条尺现在只有 journald 一条是实测，D3 的检查脚本会一次翻掉另外三条；⑤ 的四类演练在 D4 之前连「首个症状」都还是预测。范围与口径边界见 [`week10-visualization-plan.md`](./week10-observability/notes/week10-visualization-plan.md) §9 与 §12。
