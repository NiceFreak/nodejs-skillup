...
5. **D 展板校正**（表述 2026-08-20 修订，原「核谎」）：先拿最新数据逐格核对已有结论、不一致的当场修正（`yarn verify:board` 保持全过），再谈补内容。W10 板现在是**七块**（⑤ 已于 8/20 落地），校正重点＝⑤ 里「未实测」那四格今天是否已有实测数据该翻档、以及 ① 那格仍挂已拍板是否仍然成立；W9 板十三块的返工仍记在 `BACKLOG.md`。
