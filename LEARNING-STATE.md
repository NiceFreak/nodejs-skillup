...
- 2026-07-22，展示前端加**「展示模式 / 复习模式」全局开关**（纯前端，`localStorage` 持久化，无新 API，默认展示模式）：展示模式面向外部观众，收敛学习状态外现——W3 隐藏「仍在路上」开放问题与「套模板漏核心」自我观察，W3/W5/Auth 的副标题与徽标改中性技术措辞；复习模式面向本人，完整呈现验收状态与仍在路上的部分。`BoardMode` 类型置于 `types.ts`。
