...
2. **展板现状（8/18 更新，`yarn verify:board` 316 项全过，未部署）**：
   ...
   - **W10 板（tab `可观测性`，reviewOnly）落地 2/6 块**：⑥ 假生效、① 盲区已做；**③ 日志旅程、② 字段销账、④ 阈值尺、⑤ 演练分档待做**。按方案 §9 的顺序纪律「先做的会先说谎」：③② 何时做都不亏（D3 不碰它们的格子），**④⑤ 押到 D3 / D4 之后**——那时四条红线与四类演练才从 `contract` 翻成 `measured`。范围与口径边界见 [`week10-visualization-plan.md`](./week10-observability/notes/week10-visualization-plan.md) §9 与 §12。
