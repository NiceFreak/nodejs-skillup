...
- **2026-08-19（W10 D4 规划起草 · 仅文档，零副作用）**：建立 [`day4-fault-drills.md`](./week10-observability/notes/day4-fault-drills.md)——§1 把「每一类都能说出先看哪一个、为什么先看它」这句验收句拆成**六条可证伪判据**（类数 ≥3 / 每类至少一条命令输出 / 定位顺序注入前写死 / 事实分层 / **监控表态销账** / 基线全恢复且残留清零）；§2.0 把 D3 §9.5 的 `check-cert.timer` 频率修正定为**开工前置**（销账方式是 `list-timers` 的 NEXT/LAST 间隔，不是读文件）；§2.3 前置四件事逐类核验表（止步与回滚两列照抄 8/17 契约原文，还原点与基线时刻现场填）；§2.4 三层回滚 + 残留核零清单；§2.5 五条止步线。**§3 P1–P5 全部留空待本人作答**：P1 证书那一类接力还是重复（三条出路 + 代价）、P2 每类首查项与二分依据、P3 **真故障来了四项 check 会不会红**（预测全绿的那一类 = 覆盖盲区）、P4 等真实 timer 触发还是手工跑（磁盘项最坏等 59 分钟）、P5「恢复算恢复」的核零判据。§4 时间盒 A–I（约 4h15，砍单顺序写明、块 A 与块 H 不许砍）；§5 只放白名单语法；§6 五段式空模板 ×4（含「注入前预测」与「预测 vs 实际」两段）。**同时报告一处冲突**：契约 §5.4 第 4 类的注入方式与 D3 §9.2 ⑤ 已执行的弄红手法相同，D3 §3 P5 留的「走正式路径」又与 `week10-plan.md` §8「不碰现网证书」打架 —— 已落成 §3 P1，**AI 未替选、未改契约**。`week10-plan.md` §4 D4 条目同步补链接与执行清单。**当天零副作用**（未连服务器、未改配置、未注入任何故障）。
...
0. **W10 D3 已完成（8/19）**。**W10 D4 = 当前主线（8/20）**：故障演练主场（3–5 类真注入），当日规划已起草 [`day4-fault-drills.md`](./week10-observability/notes/day4-fault-drills.md)（§2 变更单含 §2.0 开工前置 / §2.3 前置四件事逐类核验表 / 三层回滚 / 五条止步；§3 P1–P5 **待本人作答**；§4 时间盒 A–I；§6 五段式空模板 ×4）。按契约 §5.4 分档表逐类走「注入 → 现象 → 定位 → 修复 → 恢复基线」。D3 已提供三项硬依赖：① 四项检查都已红过（D4 定位靠它们表态）；② 验证⑨ 三层基线绿（无绿基线不许注入）；③ 今天用掉的弄红方式已记清（D4 注入不得与 D3 重复，证据接力）。第一个动作：按 §5.4 逐类核前置四件事（还原点 → 基线 → 止步 → 回滚命令）；**端口占用类先确认服务器上 socat / nc 哪个已装**（D1 Q13 遗留确认项）。**D4 开工前先修一件**：`check-cert.timer` 的 `OnCalendar` 单位写错（每 6 分钟 ≠ 每 6 小时，见 D3 §9.5），改完用 `list-timers` 的 NEXT/LAST 间隔销账——D4 一整天靠这四个 timer 表态。**起草 D4 规划时报告的一处冲突（待 D4 §3 P1 拍板）**：契约 §5.4 第 4 类「证书过期（判定逻辑模拟）」的注入方式与 D3 §9.2 ⑤ 已执行的弄红手法是同一个（假证书 + `CERT_OVERRIDE`），重复即无新证据；而 D3 §3 P5 给 D4 留的出路「走正式路径」与 `week10-plan.md` §8「不碰现网证书」冲突 —— 三条出路（降为 3 类 / 换不碰现网证书的接力形态 / 走正式路径并改止步线）已列出代价，AI 未替选。D3 遗留观察：check-cert 的 openssl stderr 行混入 NDJSON 流（非阻断，接 Promtail/Vector 前处理）。D2 遗留观察仍开放：本地 jest 并行竞态（--runInBand 串行 9/9 过，多 suite 共享外部库互相 dropDatabase，非今日引入）。
1. **W9 收口清理（非主线）**：
...
2. **W10（当前周）**：[`week10-plan.md`](./week10-observability/notes/week10-plan.md)（§3.1 演练安全边界 / §5 决策清单 / §6 黑白名单）；[`day4-fault-drills.md`](./week10-observability/notes/day4-fault-drills.md)（**D4 当前主线规划**：§2.0 开工前置 / §2.3 前置四件事核验表 / §3 P1–P5 待答 / §5 白名单命令 / §6 五段式模板）；[`day3-monitoring-alerting.md`](./week10-observability/notes/day3-monitoring-alerting.md)（**D3 已收口**：§9 执行记录 / §9.5 timer 频率单位错与处置 / §10 明日入口）；[`day1-observability-contract.md`](./week10-observability/notes/day1-observability-contract.md)（**已冻结契约**：§5.3 四项判据表 / §5.4 演练分档表）；[`day2-logging-rollout.md`](./week10-observability/notes/day2-logging-rollout.md)（**D2 已收口**：§2 变更单 / §3 P1–P5 答案 / §11 实测 vs 期望）；`week10-observability/notes/nginx/`（四份 site + log_format 副本 = 服务器可追溯保存点）。
...
- **2026-08-19（W10 D4 规划起草）**：AI 起草 `day4-fault-drills.md`——变更单与核验表**结构**、时间盒排布、§5 的 `systemd-analyze` / `curl` / `fallocate` / `openssl` **语法与工具行为**（白名单），§3 P1–P5 的**问题与追问**、§6 五段式空模板。演练清单的选择与安全分档、定位顺序、根因推理、盲区判断、恢复判据全部属黑名单（`week10-plan.md` §6），**AI 只列代价与冲突、未替选、未代填**。review 报告一处契约与 D3 执行记录的冲突（证书那一类已被 D3 用掉 vs「走正式路径」违反不碰现网证书），落成 §3 P1 交本人拍板。未触发 `DEBT.md`（白名单语法 + L1/L2 出题，黑名单零实现）。
