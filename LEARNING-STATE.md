...
> 最后更新：2026-08-24（Asia/Shanghai，W11 D1：周计划草案 [`week11-ci/notes/week11-plan.md`](week11-ci/notes/week11-plan.md) 已建立，§5 十八条决策待拍板，尚未产生任何副作用）
>
> 上一次更新：2026-08-22（展板五轮改动：补齐 W1–W2 缺口新增「服务端架构」板、数据库板扩到八个知识点，四块从表格改成流动图，形态倒回 W4 / W6 / OAuth2 / W6Day4 四块存量，W10 数据时效核查。**W11 入口与两项移交不变**。2026-08-21 W10 全周与展板文案收口完成；W9 / W10 笔记与 runbook、以及八块展板与面试准备板的可见文案已按 `TECHNICAL-WRITING-PROTOCOL.md` 核查并整改，其他周的历史笔记仍不批量优化）

## 当前进度

- 当前周：**W11（8/24–8/28，CI 流水线与自动化发布）**——W10「可观测性与线上排障」已于 **2026-08-21（D5 收口日）全周收口**。
- 当前 Day：**W11 D1（8/24 周一）进行中**——周计划草案已建立（[`week11-ci/notes/week11-plan.md`](week11-ci/notes/week11-plan.md)），§4 决策清单 **0 / 18 已拍板**（问题库在 [`day1-release-contract.md`](week11-ci/notes/day1-release-contract.md)，编号与计划 §5 同号）。D1 是零副作用日：契约冻结前不装 Jenkins、不在服务器上建任何用户或密钥、不改配置。8/22–8/23 周末为无人值守期，W10 D5 块 H 已核零。
...
1. **答 [`week11-ci/notes/day1-release-contract.md`](week11-ci/notes/day1-release-contract.md) §4 的 Q1–Q18**（周计划与 D1 契约草案均已于 8/24 建立）。执行顺序按该文件 §3 时间盒 A–F：先读 §2 事实与 §2.5 前置概念，**块 C 的只读采集要在答题前做完**（Q6 / Q9 / Q12 没有基线答不了），答完过 §4.1 的九对冲突自查、填 §5 的五张表，本周状态改为「契约冻结完成」。
2. 十八条里先决的三条：**Q2**（controller 在开发机、没有公网入口时「推送后自动构建」怎么成立，它决定交付成果①的验收句）、**Q8 / Q9**（部署身份与可执行命令清单，W9 以来第一次把可写权限交给自动化）。
3. 排在 D3–D5 的既定项：类 2「假 active」最小样本复现（W10 唯一悬案，排 D4）、盲区②补监控口径（Q15，载体是部署后验证）、Java stretch 与 Maven 构建 job 合并执行（8/17 拍板，主线收口后才开始）。
