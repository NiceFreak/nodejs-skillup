...
- 当前周：**W4 · 认证与鉴权**
- 当前 Day：**D3 JWT 专注日；签发已验收，下一步设计验证中间件**
...
1. 单独定义 JWT 验证中间件契约：token 来源、成功输出，以及缺失/格式错误/篡改/过期分支。
2. 本人实现验证中间件，不把具体业务权限写进认证逻辑。
3. 选择一个真实接口受保护，并用 Postman 保存的 `accessToken` 验证有/无有效 token 两条路径。
