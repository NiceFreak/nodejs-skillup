...
- **独立线（不与学习主线同步开展，2026-08-20 起）**：**展板状态核查线**——阶段一 **1/8**（只判定不改；8/20 已判第 1 块「认证与授权」）。执行文档 [`showcase-audit-line.md`](week8-fullstack/notes/showcase-audit-line.md)；排队不排期，只在当天主线达到止步条件且有 15–20 分钟空档时推进，零进展不算欠账。**不属于并行线**（8/17 拍板的并行线仍只有英语 + 项目叙述）。
...
- **展板状态核查线（白名单，8/20 晚）**：本人说「开启展板状态核查」→ 按核查线 §6 触发，主线已收口故可进入；
  只做第 1 块「认证与授权」，AI 出判定与可复核依据（组件结构、`stopAt` 取值、字段与字数统计），**零代码改动**。
  「遮住 `h3` 能否答出验收句」那道闸的强制力等级仍是「人」，故 §5 那一行标为 AI 预判、待本人复核。
