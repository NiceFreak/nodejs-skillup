...
D3 开始时先定义登录契约和失败语义，再进入代码：

1. 明确登录成功、邮箱不存在、密码错误分别如何响应，避免泄露账号是否存在。
2. 设计 `findByEmail` 如何显式取回默认隐藏的 `passwordHash`。
3. 复述 `bcrypt.compare` 的输入输出和职责位置。
4. 明确 JWT payload、过期时间、secret 来源和受保护接口，再实现登录签发与鉴权中间件。

## 当前阻塞与风险

- W4 剩余 D3–D5 三个有效学习日。
- D3 同时包含登录、JWT 和鉴权中间件，范围较满；当天必须至少保护一个真实接口。
