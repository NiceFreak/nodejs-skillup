...
- 当前 Day：**W11 D1（8/24 周一）进行中**——周计划草案已建立（[`week11-ci/notes/week11-plan.md`](week11-ci/notes/week11-plan.md)），§5 决策清单 **0 / 18 已拍板**。D1 是零副作用日：契约冻结前不装 Jenkins、不在服务器上建任何用户或密钥、不改配置。8/22–8/23 周末为无人值守期，W10 D5 块 H 已核零。
...
1. **答 `week11-ci/notes/week11-plan.md` §5 的 Q1–Q18**（计划草案已于 8/24 建立），答完落成 `week11-ci/notes/day1-release-contract.md`，本周状态改为「契约冻结完成」。
