...
- 当前周：**W4 · 认证与鉴权**
- 当前 Day：**D3 JWT 专注日；签发、验证中间件和受保护报表已闭环，剩余自然月边界重建**
...
1. 完成 D3 排定的 15–20 分钟第一档重建：自然月报表边界，过程中 AI 不提示，只在结束后验收。
2. 学习结束时把 `week4-auth/notes/day3.md` 压缩为最终 JWT 签发与验证契约、实现结果和关键误区。
3. D4 再进入 RBAC、认证失败路径测试与响应信封迁移，不在 D3 继续扩展。

## 当前阻塞与风险

- W4 剩余 D3–D5 三个有效学习日。
- Login 凭据验证已提前完成，D3 已减负为 JWT 专注日（签发 + 中间件 + 受保护接口）；当天仍必须至少保护一个真实接口。
- 三条欠债待安排第一档重建：D3 做自然月边界，D4 做注册调用链；JWT 签发链路在 D4 或周验收前完成，不留到周五集中处理。
