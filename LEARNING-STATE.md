...
- **2026-08-13（D5 前置之三：展板 9 块 → 10 块 + 讲稿对齐）**：
  - **触发**：本人提问「刚推进新进度，是否配套更新 day5-demo-script.md 和 week8 可视化前端」。
  - **核查结论**：讲稿是「缺新事实」（自检只验三个面、未提 8081 门禁），W9 展板是**谎报**——
    `w9Facts.ts` / `W9Board.tsx` 停在 `f62fd7f`（事实截到 D4-HTTPS），D4-c 之后「三面」已不实为四个面。
  - **展板修复（必然）**：PUBLIC_FACES 补 8081 面、PORT_ROWS 加 8081、CHAIN_NODES/ufw/ACC_SEGMENTS 四面对齐、
    SITE_CONFIGS 加 shop-showcase、PRODUCTION_PARITY 补 D4-c、认知修正 14→17 条（服务边界/产物分目录/门禁只挡浏览器）。
  - **第 10 块「服务边界 vs 暴露边界」**（用户决策单开）：w9Facts 增 SERVICE_EXPOSURE/EXPOSURE_SPLIT，
    w9Topics W9_STAGE_PLAN 9→10，W9Board 新增 ServiceExposureBoard（一进程四个门）、styles 配套样式。
  - **讲稿（推荐）**：自检「三条→五条」（补 8081）、Act 3 补 8081 明文短板，与「四个公网面」一致。
  - **验证**：`yarn typecheck` 通过；`VITE_SHOWCASE_ONLY=1 yarn build` 通过。未发布 Pages（该仓库不在本次会话）。
  - **AI 边界**：week8 展示前端属白名单（AGENTS.md），AI 直接实现；未触发 `DEBT.md`。
