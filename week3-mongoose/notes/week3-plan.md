...
- ✅ 一篇查询优化笔记：能讲清各管道阶段、索引对 `explain` 的影响 —— **已成篇**：Day 1（`COLLSCAN → IXSCAN` + 复合索引）+ Day 4（`$lookup` 关联性能判读,见 `day4-lookup-index-query-optimization.md`）。
- ✅ 掌握判据：能**脱离 AI 从空白重建**一个聚合 demo —— **已达成**（月度趋势报表为独立设计）。

> **本周进度小结（截至 Day 4 / 7/9）**：主线聚合 + 测试**已提前完成**（含原计划 Day 5 的量);查询优化笔记 Day 4 已成篇（`$lookup` 关联性能）。剩余待办:①周复盘中文+英文稿（提纲已备）②Day 5 demo 自测（讲稿已备）。
