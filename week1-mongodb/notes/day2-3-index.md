...
**Q:加了索引后,哪个字段能看到扫描/耗时的减少?**

用 `explain("executionStats")` 看,重点关注这几个字段(按可靠性排序):

- **`totalDocsExamined`** —— 扫描的文档数。**最硬的证据**:没索引时全表扫(如 50000),加索引后可能降到个位数。不受机器负载 / 缓存影响,加了索引就是断崖式下降。
- **`totalKeysExamined`** —— 扫描的索引键数,配合上一个看。
- **`winningPlan.stage`** —— `COLLSCAN`(全表扫描)→ `IXSCAN`(索引扫描)。判断"有没有用上索引"最直观的一眼。
- **`executionTimeMillis`** —— 总耗时(毫秒)。会看,但**不是首选证据**:时间受机器负载、缓存、数据量波动影响,同一查询多次跑数字会变;小数据集上加索引前后时间差可能很小甚至偶尔变大(索引有额外开销)。

**一句话结论(被问时这样答):**
> 时间看 `executionTimeMillis`,但更能证明索引生效的是 `totalDocsExamined`——扫描文档数从几万降到个位数,这个比时间稳定,不受缓存和负载干扰。

**为什么不能只看时间:** 时间是"结果",会被环境噪声污染;`totalDocsExamined` 是"工作量",直接反映查询引擎干了多少活,是索引是否生效的因果证据。演示时用 COLLSCAN→IXSCAN + docsExamined 下降,比时间数字更有说服力。
