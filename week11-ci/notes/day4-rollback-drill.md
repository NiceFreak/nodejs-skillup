...
| G7 | **主线 A 的触发链完全依赖开发机→github.com 的网络**。D3 收口记录：当前网络路径存在 TLS 层拦截，`curl https://github.com/` = 000；Jenkins 的 Checkout SCM 与 Poll SCM 都要从开发机拉取。坏提交推不上去、或推上去 Jenkins 拉不下来，主线 A 不能开始。这一条没有仓库内技术对策 | D3 §4 网络诊断 | §2.5 止步条件 1 |
...
- **步 4（部署前基线）**：完成——deploy key `verify` 七项全绿（/health `{"status":"ok"}`、业务 `Hello, World!`、mongosh `{ok:1}`、ss:3000 LISTEN、check-app OK、check-disk OK）；controller 侧 `curl -f https://demo.example.com/` = 200；五面全 200。
