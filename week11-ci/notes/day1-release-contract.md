...
| 3 Test | controller（开发机） | `npm test`（单元 + 两份集成，MMS 在 controller 侧运行）。**MongoDB 来源：MongoMemoryServer（2026-08-25 P6 拍板回填）**——无 `MONGODB_URI` 且 `CI` 未注入时 fallback；与 Actions 的 `mongo:7` 容器不同源，定位为 Jenkins 侧隔离验证 | `npm test` 非零（用例失败、MMS 拉起失败） | **未动** | 流水线标记失败，打印失败报告，人工修复代码；无需回滚 |
