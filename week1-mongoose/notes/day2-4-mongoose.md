...
- **`required` / `min` / `max` / `enum` 是 Mongoose 的"应用层校验"**:写入数据库**之前**就在代码层检查,抛 Mongoose 的 `ValidationError`(友好报错), 此时数据并不会真正进入数据库
- **`unique` 不是校验器,而是"数据库层约束"**:靠 MongoDB 的唯一索引,要真的尝试写入、由数据库拦下,报错 `E11000`。
- **三个实际后果**:
...
总结:**Mongoose 校验(写入前、友好报错)和数据库约束(唯一索引、`E11000`)是两层不同的防线,防线不同、报错不同、处理方式也不同。**
