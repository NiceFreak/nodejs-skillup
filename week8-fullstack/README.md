...
- **服务端架构页**：六个知识点按请求的走向排列——中间件管道（进入正序 / 离开反序的逐帧输出、`next()` 二选一、`next()` 之后 0ms 与 `res.on('finish')` 100ms 的计时点对照）、四层职责与依赖方向（碰什么 / 不碰什么，附「洋葱模型 ≠ 分层单向」的区分）、一条 `GET /users/:id` 穿六格的形状变化与三种结局（400 / 404 / 200 各停在哪一层）、两条互斥的响应路径（`null` 走 controller、`throw` 走错误中间件）、错误翻译落层矩阵（七类原始错误 → 翻译点 → 领域错误 → 状态码，附 Mongoose 校验与数据库唯一索引两层防线）、归属判断标尺（描述 HTTP 形态还是业务规则，附三条被推翻的做法）。事实以 `week2-express/src` 当前代码为准，与笔记不一致处在证据里点明。可用 `#/showcase?tab=architecture&topic=request-shape` 直接进入。
- **数据库聚合页**：八个知识点分三段——建模决策（嵌入 / 引用 / 快照的四个维度与订单系统四个决策）、聚合分层、customer spending 六阶段 pipeline 形状、自然月边界，以及 explain 三数、复合索引最左前缀（A/B/C 三查询 16667 / 334 / 1000 的扫描量对照与删索引反证）、覆盖查询（`PROJECTION_COVERED` 正向与去掉 `_id: 0` 的反证）、`$lookup` 外键索引；复习态逐步揭示下一 stage，事实 / 能证明 / 没有证明与图相邻保留。W1 三块的实验跑在 mongosh 手工造的 `bigdata` 集合上，口径标注与图相邻。
