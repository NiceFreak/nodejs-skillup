...
- **内部学习展板 `#/showcase`**：无需登录，可查看认证与授权、OAuth2、服务端架构、数据库聚合、Node.js 运行时、测试闭环、排障手册与学习笔记（面试准备、部署上线、可观测性、发布流水线只在复习状态出现）；
  认证页先以同一条端到端链路对照 401 / 403 / 200 的停止点与职责归属：顶部是三条路径 × 七段的停止点矩阵，下面是四泳道序列图——每段消息画成一根跨列箭头，起点列是发出方、终点列是接收方，载荷骑在箭头上，因此「长期密码只在前三段」「受保护请求整段停在管道列内」看得出来而不必读文字。展示状态直接给出完整链路；复习状态先要求口述两道门和停止层，再展开核对。两种状态是认知任务区分，不是权限隔离。
- **学习演进与关系图例**：导航下方固定呈现 W1–W2 服务端架构 → W1·W3 数据与查询 → W4 身份边界 → W5 运行时 → W6 测试证据 → 产出「讲得出口」；全站统一使用一处调用流、资源、成功、受控拒绝、异常与未测量图例。
...
- **OAuth2 流程页**：三泳道序列图，六段消息各画一根跨列箭头，起点列是发出方、终点列是接收方。
  前信道与后信道由几何区分——箭头触到「浏览器」列即经过浏览器，只连「后端」与「第三方」两列即后端直连，
  因此「`client_secret` 绝不经过浏览器」表现为第 4、5 段压根不碰第一列；同时显示
  `state/code/access token/client_secret/JWT` 边界。复习态按当前起点 / 终点先预测信道和凭据，再揭示当前消息。这是 W4 的讲解型 demo，不接真实第三方登录。
- **服务端架构页**：六个知识点按请求的走向排列——中间件管道（三层同心环 + 一枚令牌进到最内层再原路退出，附 `next()` 二选一与两个计时点 0ms / 100ms 的对照）、四层职责与依赖方向（碰什么 / 不碰什么，附「洋葱模型 ≠ 分层单向」的区分）、一条 `GET /users/:id` 的双轨流动图（六条泳道、左下行右上行，令牌带着当前携带的数据形状走完一个来回，三种结局是同一条轨道上的三个停止点：400 停在校验中间件，404 与 200 停在 controller）、两条互斥的响应路径（`null` 走 controller、`throw` 走错误中间件）、错误汇聚图（七类原始错误按翻译点分三列 → 汇进 `app.js` 唯一的错误出口 → 扇出成 400 / 401 / 403 / 409 / 500，附 Mongoose 校验与数据库唯一索引两层防线，逐行明细可折叠）、归属判断标尺（描述 HTTP 形态还是业务规则，附三条被推翻的做法）。事实以 `week2-express/src` 当前代码为准，与笔记不一致处在证据里点明。可用 `#/showcase?tab=architecture&topic=request-shape` 直接进入。
- **数据库聚合页**：八个知识点分三段——建模决策（嵌入 / 引用 / 快照的四个维度与订单系统四个决策）、聚合分层、customer spending 六阶段 pipeline（每一步并排画出前后两张文档卡，字段芯片按新增 / 消失 / 改形着色，卡底单列「一条文档代表什么」的多重性变化）、自然月边界，以及 explain 三数、复合索引最左前缀（A/B/C 三查询 16667 / 334 / 1000 的扫描量对照与删索引反证）、覆盖查询（`PROJECTION_COVERED` 正向与去掉 `_id: 0` 的反证）、`$lookup` 外键索引；复习态逐步揭示下一 stage，事实 / 能证明 / 没有证明与图相邻保留。W1 三块的实验跑在 mongosh 手工造的 `bigdata` 集合上，口径标注与图相邻。
...
- **测试闭环页**：Day 1 并排对照“真实 admin Token → 报表 200”和“新注册用户 → 报表 403”两条链，用覆盖拓扑、证据矩阵和资源时间轴说明 fixture / `JWT_SECRET` / 内存库生命周期以及状态码边界；Day 2 展示 CI / 本地三路数据库来源、共享 `mongo:7` 下的 suite 级逻辑库隔离、`Model.init()` 初始化等待点、teardown 顺序与远端 CI 证据。Day 3 用同一条八段跨层轨道对照 admin `200`、member `403`、无 Token `401` 的停止点，分开呈现调用顺序、职责归属与返回值来源；轨道按 U 形折返排布——01–07 请求出站排一行，08 React render 落回第二行第一列并用虚线画出回程，六个交接点各挂一枚令牌显示这一段交出的值。Day 4 把 W3–W6 收束成一条论证：四维框架（契约 / 职责 / 证据 / 限制）→ 因果主线 → 十层交接链（九层依次交接各挂一枚交付物令牌，十个令牌两两不同即是「没有两层交出同一种东西」的证据；全局 error handler 画成缩进的侧支，它不是第十步）→ 三组受控实验 → 九条关键纠错 → 六条当前限制 → 能力归属时间线 → 最终验证。四个 Day 都支持复习状态先口述再展开核对，可用 `#/showcase?tab=testing&topic=day4` 直接进入 Day 4。
- **面试准备页（仅复习状态）**：这是复习材料，不进对外 demo —— 展示状态下这个 tab 不出现，带 `tab=interview` 的链接会自动进入复习状态，在该页切回展示状态则落回默认页。把 `interview-prep/` 两份问答稿的结论做成五块可视化——强项分层（面试往哪引，三档处理方式贯穿全板）、
...
- **学习笔记页**：直接读取 `interview-prep/` 两份问答稿、本 README、`notes/` 下 Markdown、W1 与 W2 的七份原文（建模 / 索引 / Mongoose / D1–D4）与 W6 心智模型；
