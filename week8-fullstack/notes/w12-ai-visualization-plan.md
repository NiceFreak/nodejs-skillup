...
> 深链） / `yarn audit:visual` + 桌面/手机截图 + 人工视觉验收（逐块 expected answer / 标题+结论段遮挡 /
> 首屏结论锚）+ 存量首屏回归。
...
| 人工视觉验收 | 逐块记录：expected answer（用 §4 各块验收句）→ **标题与结论段同时遮挡** → 观察回答 → 记通过/不通过；**首屏是否先出现完整视觉舞台/结论锚**（根规范 §4.1/§6）；10 秒结论；视觉记忆点来自技术关系 |
...
- **M3**：C1 mock 实验（D4）实测数字回填 B4 → 视觉复核 + 截图 + 人工视觉验收记录 → 验收。
- 风险：
...
5. 验收是否仍完整覆盖：图拓扑、首屏结论锚人工视觉验收、深链回退、存量回归三项判据？
...
| 实现方与独立 review 视觉检查 | P4/B2/B3/B4/B5 两档视口通过；P4/B4 首轮文字碰撞修复后复拍无重叠。仓库主人的标题遮挡测试仍须独立执行，不能由实现方代替 |
...
| ⑩ 验收证据 | 结构断言核对七类记录、四类默认渲染、三类丢弃、共同前置、custom / fallback 从第二阶段后分叉、历史与本轮输入双来源、读写闭环和两条路径帧序；桌面 `1440x1000`、手机 `390x844` 核默认态与展开态；仓库主人隐藏标题与结论，仅根据图示复述关键关系，并逐项记录通过或卡点。2026-09-06 本人确认该项完成。 |
...
- Bub 离线源码证据：**2026-09-06 完成**。为满足“离线且只 clone 本仓库即可复核”并限制体积，加入
  37,503 bytes 的压缩证据包，只含报告引用的 11 个源码文件、`pyproject.toml` 与上游 `LICENSE`；
  完整 commit、SHA-256、解压方法和范围边界见 `week12-python-rag/vendor/README.md`。
- 论断 10：**延期**。真实 TCP/TLS 的 `asyncio.timeout` 取消与 httpx 连接池复用实验保留为
...
- B3 图示人工验收：实现方完成了 10 秒结论、首屏路径与视觉记忆点的图像复核；仓库主人负责隐藏标题与
  结论后仅根据图示复述关键关系，并于 2026-09-06 确认该项完成。

---

## 12. 概念地图总览块（新增第 9 块，十列契约已验收，可进入实现）

> 状态：设计草案 v0.4（2026-09-05，第三轮 review 四项同步完成，可进入实现）。v0.4 修正：
> ① 假设来源边 2.4→2.5 证据档改为「本人实测」（C1 假设取自 §7、等价实验由本人执行属
> 实测；「不能证明本体运行」是外推边界不降档），推断边改为 2 条；② ②/⑩ 同步推断集合；
> ③ §3.1 五条原文按源文件逐字 blockquote 引用并标行号，不再缩写改写；④ AeGroup 边界
> 修正：导航第三组「概念地图」扩展 AeGroup，topic 级「混合」只扩展 evidenceKind
> （候选 A）。冻结项：id concept-map、label 总览、导航组名概念地图、evidenceKind 候选 A。
> 内容源 = `week12-python-rag/notes/w12-concept-map.md` v1.0（已验收）：§2 五张导航卡 =
> 五个对象节点；§3.1 五条关系 = 关系边。本节只做设计，未实现任何页面。
> 落点 = 方案 A：`ai-engineer` 板新增总览块（第 9 个 topic），复用现有导航 /
> 深链 / 断言基建。
> 视觉依据：`SHOWCASE-VISUAL-PROTOCOL.md`；文案依据：`TECHNICAL-WRITING-PROTOCOL.md`（含 §3.5）。

### 12.1 形态推导

**内容核心关系**（先于一切形态判断）：概念地图不是比较两列语法，也不是某个运行时机制的状态机，而是
「五类认知对象 + 它们之间已识别的连接」。连接类型不止一种：有载体、有迁移、有范围包含、有实例化、
有假设来源。关系集合开放，不暗示穷尽。

**认知任务**：让读者一眼看出（1）五个对象分别是什么；（2）它们之间存在五类已识别关系、以七条带方向的边呈现；（3）Bub 是同时以三条实例化出边联系 2.1/2.2/2.4、并接收一条假设来源入边的交汇实例（这是内容源 §2.5 支持的结论）；（4）这些关系不是完备分类。

**形态候选**：带类型标签的有向关系图（节点 = 对象，边 = 关系类型）。不用单一流程线（关系不是时序）、
不用单一矩阵（关系类型异构、不是同一维度上的两两比较）、不用强制同心嵌套（会暗示完备层级，违反
§3.5）。
...
| 列 | 内容 |
|---|---|
| ① 单一问题 | W12 的五个学习对象之间已识别出哪些连接关系？Bub 在其中处于什么位置？ |
| ② 10 秒结论 | 五个对象之间存在五类已识别关系，画成七条带方向的边：载体（2.1→2.2）、方法迁移（2.2→2.4，推断）、范围包含（2.3→2.4，推断）、实例化（2.5→2.1/2.2/2.4 三条）、假设来源（2.4→2.5，实测）。推断边共两条（方法迁移、范围包含）。Bub 是同时以三条出边联系 2.1/2.2/2.4 并接收一条入边的交汇实例。关系集合开放，本图只画已识别的。 |
| ③ 对象与数据形状 | 节点 = 5 个对象（id 2.1–2.5）。边 = 七条，分属五类关系（载体 1、方法迁移 1、范围包含 1、实例化 3、假设来源 1）。每条边带：from/to、关系类型、证据等级、方向箭头。图例 = 五类关系线型 + 证据等级标记。 |
| ④ 结论编码 | 节点空间位置编码「分区归属」：左区 = Python 与工程能力（2.1/2.2），右区 = AI 工程、Agent 与 Bub（2.3/2.4/2.5）。Bub（2.5）属于右区，置于右区左缘（靠近两区分界），不越界到左区；分区位置与组内相对位置是两层：前者编码归属，后者表达它横跨联系两个分区。边线型编码关系类型，五类分五种线型。边方向由箭头承担。证据等级不用线型（避免与关系类型争同一通道），改用边标签旁的独立标记（如「推断」后缀）与图例分区表示。2.4 与 2.5 之间是一对反向边（2.5→2.4 属实例化，2.4→2.5 属假设来源），用弧形偏移分离，不重叠为同一直线，两个箭头方向各自可读。颜色只作第二编码。 |
| ⑤ 视觉舞台 | 首屏一张关系图：左区「Python 与工程能力」（2.1/2.2），右区「AI 工程、Agent 与 Bub」（2.3/2.4/2.5），中间为关系边与图例；Bub（2.5）置于右区左缘。10 秒结论与边界句常驻图上/下方。 |
| ⑥ 文字层级 | 常驻：节点短名、边类型短标签、方向箭头、证据标记（仅推断边加「推断」后缀）、图例、10 秒结论、边界句（集合开放）。折叠：每条关系的完整依据（§3.1 原文引用）、来源文件、证据等级细则。 |
| ⑦ 视觉记忆点 | Bub 节点（右区左缘）同时向 2.1/2.2/2.4 发出三条实例化出边，并从 2.4 接收一条假设来源入边。图面位置表达「一个实例横跨 Python 与工程能力、AI 工程与 Agent 两区」这一几何事实，记忆点来自 2.5 的实例化角色本身，无比喻与装饰。 |
| ⑧ 图标策略 | 不新增图标。对象用短名 + 编号（2.1–2.5），关系类型用图例线型；节点形状不引入额外语义。 |
| ⑨ 动效策略 | 静态关系图为主；若实现 hover，只做「高亮某节点的关联边」这类方位保持过渡，不循环、不伪装时序过程（关系不是流程）。reduced-motion 下完整保留节点与边。 |
| ⑩ 验收证据 | 结构断言：5 节点存在且 id 正确；7 条边（载体 1、迁移 1、包含 1、实例化 3、假设来源 1）；每条边 from/to 方向与 §12.3 表一致；推断边集合 = 确定的 2 条（2.2→2.4、2.3→2.4），这 2 条带「推断」标记，其余 5 条不带；图例含五类线型与证据标记；边界句出现「开放」；Bub 节点出边 = 3。视口与状态：桌面 1440×1000、手机 390×844，各核默认态；有折叠时补全部展开态；有复习门时补复习态。度量：默认态屏数与主路径中文字数（目标 ≤1.5 屏、≤220 字，超出记录不可拆理由）。截图：桌面与手机默认态截图存档，指明路径；布局或内容变更后重新截图。人工视觉验收（逐项记通过/不通过）：遮住标题与结论段后能从图说出五对象与五类关系；10 秒结论；视觉记忆点来自技术关系；首屏先出现完整视觉舞台而非连续正文。 |

### 12.3 数据层草案（对应 `aiEngineerTopics.ts` 新增 kind）

```ts
// 草案，review 通过后才实现
// 证据接线：edges[].evidence 用仓库四档 AeEvidence 的完整拼写
// （源码事实 / 本人实测 / 推断 / 待运行验证），不使用「仓库事实 / AI 推断」二档，
// 避免与 AeBase.evidenceKind 及通用渲染产生第二套证据词表。
// 总览块整体证据徽标 = 「混合」（§12.6 已冻结）：topic 级汇总状态，不加入 AeEvidence，
// 不扩展 AeGroup。类型接线采用候选 A：`evidenceKind: AeEvidence | "混合"`
// （2026-09-05 review 裁定，见 §12.6）。
export interface AeConceptMapTopic extends AeBase {
  kind: "concept-map";
  nodes: Array<{ id: "2.1" | "2.2" | "2.3" | "2.4" | "2.5"; label: string; zone: "left" | "right"; note: string }>;
  edges: Array<{
    from: string; to: string;
    relation: "carrier" | "migration" | "contain" | "instantiate" | "hypothesis-source";
    evidence: AeEvidence;
    note: string;
  }>;
}
...
对象清单与 §3.1 五条关系的映射：下表逐字引用 §3.1 原文，每条边证据取唯一值。
「推断」边 = 2 条（方法迁移 2.2→2.4、范围包含 2.3→2.4）。

**§3.1 原文（完整引用，逐字取自 `w12-concept-map.md` 第 96–100 行，未改写）：**

> 1. **2.1 → 2.2 载体关系**：Python 工程能力是代码阅读与排障的载体；D3/D4/D5 的阅读、预测对照与诊断全部在 Python 代码上进行（依据 2.1 已有材料、2.2 已有材料）。
>
> 2. **2.2 → 2.4 方法迁移关系**（AI 推断，非仓库直接事实）：排障方法论（假设 → 执行 → 验证 → 归因）来自 W9–W11 的 shell/systemd/Node 实践归纳，是仓库自有的方法表达；D3/D4/D5 把它用于理解 Python async 生命周期与资源清理。它是否适用于 Agent 运行时（循环、取消、收尾）属跨材料推断。外部佐证方向一致：Anthropic《Building effective agents》将 agents 定义为「LLMs dynamically direct their own processes and tool usage」，与 workflows（predefined code paths）相对；这与 Bub 报告 §0.1 术语表对 agent/workflow 的区分一致。该外部资料只佐证「agent 是模型动态指挥、runtime 提供编排」的方向，不构成对本仓库排障方法的规范命名。OpenAI running-agents 文档因访问受限（403）未直接核实，review 转述不作为已验证证据写入。
>
> 3. **2.3 → 2.4 范围包含关系**：agent 是 AI 工程内的一种架构范式，不是 AI 工程的全部；同一领域还包含检索、评估、协议与工具层（依据 2.4 一句话定位、五周计划能力目标）。
>
> 4. **2.5 对 2.1/2.2/2.4 的实例化关系**：Bub 同时是真实 Python 代码（2.1 的阅读对象）、排障方法的应用现场（2.2 的练习载体）、agent runtime 实例（2.4 的源码参照）（依据 2.5 一句话定位）。
>
> 5. **2.4 → 2.5 假设来源关系**：C1 实验的可证伪假设取自 Bub 报告 §7 C1（step 循环收敛性），实验在 day4 以 Python 等价结构完成并限定为「等价结构验证，非 Bub 真实运行」；实验结论未回填 Bub 报告 §7，该节仍为候选问题与验证方案，Bub 真实 `max_steps` 触发仍列于报告 §8 待验证。等价实验可作单元级佐证，不能证明 Bub 本体的运行行为（依据 day4 §11 C1、Bub 报告 §7/§8）。

**映射表（每条边取唯一证据值，定值依据列明）：**

| 边（from → to） | 关系 | §3.1 原文条款 | 唯一证据值 | 定值依据 |
|---|---|---|---|---|
| 2.1 → 2.2 | 载体 | §3.1 第 1 条 | 本人实测 | D3/D4/D5 的阅读、预测对照与诊断均为本人执行并记录，落在 Python 代码上 |
| 2.2 → 2.4 | 方法迁移 | §3.1 第 2 条 | 推断 | 原文标题显式标「AI 推断，非仓库直接事实」 |
| 2.3 → 2.4 | 范围包含 | §3.1 第 3 条 | 推断 | 内容源 2.3 边界注明该范围描述为推断，非权威定义；包含关系本身依据归纳性的领域描述 |
| 2.5 → 2.1 | 实例化 | §3.1 第 4 条 | 源码事实 | Bub 是真实 Python 项目（commit 33c417a），「是 Python 代码」为源码可核事实 |
| 2.5 → 2.2 | 实例化 | §3.1 第 4 条 | 本人实测 | 「排障方法的应用现场」来自 D3/D5 实际执行记录 |
| 2.5 → 2.4 | 实例化 | §3.1 第 4 条 | 源码事实 | 「agent runtime 实例」由 Bub 源码与阅读报告确认 |
| 2.4 → 2.5 | 假设来源 | §3.1 第 5 条 | 本人实测 | C1 假设取自报告 §7（源码），等价实验由本人手写并在 day4 执行完成（实测）；「不能证明 Bub 本体运行」是外推边界，不改变假设来源与实验已发生这一事实 |

> 实现前定值冻结要求：上表唯一证据值由 AI 依据 §3.1 原文与各卡定级，经独立 review
> 逐项确认后冻结（2026-09-05）。若本人调整任一档位，须同步更新上表与 ⑩ 的推断边集合。
> 实例化 = 2.5 → 2.1/2.2/2.4 三条出边；假设来源 = 2.4 → 2.5 一条入边。
> Bub 节点实际同时有出边（实例化）与入边（假设来源）。

### 12.4 落点与范围

- 新增第 9 个 topic：id `concept-map`，kind `concept-map`，label `总览`（2026-09-05 冻结）。
- 导航归属（已冻结，§12.6）：独立第三组，组名「概念地图」（2026-09-05 冻结），置于
  「Python 迁移增量」「Bub harness 骨架」两组之前，桌面占满一行。图内分区术语 =
  左区「Python 与工程能力」、右区「AI 工程、Agent 与 Bub」，与导航组名无关。
- 影响面：`aiEngineerTopics.ts`、`AiEngineerBoard.tsx`（新增 visual 分支）、`styles.css`、
  `verify-w9-board.mjs` §E（深链数 8 → 9 等）、`w12-ai-board-design.md`（八块 → 九块表述）。
- 验收命令沿用：`yarn typecheck` / `yarn build:showcase` / `yarn verify:board` / `yarn audit:visual`。

### 12.5 决策与定值状态

1. §12.3 七条边的唯一证据值已经独立 review 逐项确认并冻结（2026-09-05）：
   载体 2.1→2.2 = 本人实测、方法迁移 2.2→2.4 = 推断、范围包含 2.3→2.4 = 推断、
   实例化 2.5→2.1/2.5→2.2/2.5→2.4 = 源码事实 / 本人实测 / 源码事实、
   假设来源 2.4→2.5 = 本人实测。推断边 = 2 条。
2. 十列契约经独立 review + 本人确认，可进入实现。

### 12.6 冻结的决策（2026-09-05 按 review 裁定）

**导航归属 = 独立第三组。** 总览块作为跨组元层对象，在导航里独立成组，置于
「Python 迁移增量」「Bub harness 骨架」两个内容组之前，桌面占满一行，避免
孤立半宽分组。复用现有 topic、深链与 group 过滤结构。不选「组首」（会把跨组
对象挂到某既有组名下并引入过滤例外）；不选「特殊定位」（会扩大导航与深链接线）。

**topic 级整体证据徽标 = 「混合」。** 本块导航徽标显示「混合」，表示块内边级证据
不统一；通用证据说明文案（`AiEngineerBoard.tsx` 的 evidence note）按「混合」渲染，
不再说「本块整体为 X」。「混合」是 topic 级汇总状态，作用于证据徽标与说明文案。

**类型接线（review 裁定的边界）：**
- 导航独立第三组需要扩展 `AeGroup` union 与 `AE_GROUPS`，新增组值「概念地图」。
  这是导航组的扩展，「概念地图」作为导航组进入 `AE_GROUPS`。
- 不把「混合」加入导航组。证据徽标不属于导航组概念，「混合」不进入 `AE_GROUPS`。
- 不把「混合」加入边级 `AeEvidence`。边级证据只用四档真实值：源码事实 /
  本人实测 / 推断 / 待运行验证。
- topic 级「混合」通过扩展 `AeBase.evidenceKind` 类型容纳，采用候选 A：
  `evidenceKind: AeEvidence | "混合"`（2026-09-05 review 裁定）。既有 8 块继续用
  `AeEvidence` 单值，总览块用 `"混合"`；导航徽标与 evidence note 渲染对 `"混合"`
  走专门文案（不再说「本块整体为 X」，改为「块内边级证据混合：推断 / 源码事实 /
  本人实测」）。候选 B（`summaryKind` 扩展字段）不采用，避免同一块同时携带两个
  可能冲突的证据来源。

## 13. 概念地图阅读接线增量（v0.9，2026-09-05）

> 本节导航：[阅读入口契约](#133-阅读入口契约) / [实现结果与证据边界](#137-实现结果与证据边界)。

### 13.1 根因与范围

§12 冻结了五节点、七条概念边与图形编码，但没有定义五个对象如何到达现有八个专题，
也没有定义来源如何打开学习笔记。因此原结构断言可以全绿，读者仍然无法从总览继续阅读。

内容原点仍是 [`w12-concept-map.md`](../../week12-python-rag/notes/w12-concept-map.md#2-五个学习对象导航卡片)，
本节只记录展示板如何接入它。

本节只增加「阅读接线」和「来源深链」，不改§12 的五节点、七边、方向、证据档、分区或推断集合。
`w12-concept-map.md` 保持只读。

### 13.2 形成过程的事实口径

五个对象不是从当前八个专题反推出来，也不存在可复现的「唯一选五」算法。可确认的形成过程只有：

1. 五周能力目标和 W12 位置给出后续能力边界；
2. W12 目标、既有材料与完成产出提供本周导航素材；
3. 源文档的五张导航卡由 AI 按既有笔记和计划整理，经本人 review 定稿。

对象集合保持开放；上述过程不表示五项产出与五个对象一一对应。

### 13.3 阅读入口契约

`landingTopicIds` 是当前九块展板的阅读入口，不是对象归属、完整语义覆盖或第八类概念关系。
同一专题可从多个对象进入。

| 导航对象 | 当前阅读入口 | 编排理由 |
|---|---|---|
| Python 工程能力 | `py-syntax`、`async-failure-lifecycle` | D2 语法与 D4 async/resource 是本板直接展开的 Python 工程材料 |
| Python 代码阅读与排障 | `cli-dispatch`、`async-failure-lifecycle` | D3 代码对照与 D4 失败实验是阅读/排障的两个入口 |
| AI 工程的范围与归属 | `roles-nesting` | 职责边界是当前板的范围讨论入口，不代表领域全貌 |
| Agent 运行时概念 | `entry-chain`、`turn-pipeline`、`tape-context`、`step-loop`、`roles-nesting` | 五块分别展开入口、turn 检查点、context、step 和职责 |
| Bub 源码阅读的作用 | `cli-dispatch`、`entry-chain` | 先从 CLI 对照和启动入口读 Bub 这个真实 Python/agent runtime 参照；B2–B5 仍是 Bub 材料，由 Agent 运行时卡收敛入口 |

八个对外短标签冻结为：语法映射、CLI 分发、异步清理、启动入口、turn 检查点、tape → context、step 循环、
职责边界。P/B 只保留在历史施工记录与内部测试命名中。`2.1`–`2.5` 不再作为裸标签，只出现在可打开对应
Markdown 章节的真实链接上。

### 13.4 来源与笔记状态机

- 来源目标使用 `noteId + section` 结构，不从显示字符串猜路由。
- URL 的 `section` 只在 `tab=notes` 时合法；普通 tab/topic/索引切换均清空它。只有结构化笔记链接一次写入
  tab、topic、section 和必要的 review mode。
- AI 工程来源同时写入成对的 `returnTab=ai-engineer + returnTopic`。`returnTopic` 必须精确命中现有专题，
  才在笔记页所有内容状态上显示语义化 `<a>` 返回入口；页内跳转、同仓笔记链接、索引换篇与刷新保留它，
  离开 notes 时清空。伪造或残缺参数不回退到默认专题。
- 章节键取自真实数字标题，例如 `2.5`、`3.1`、`11`；动态正文和 heading map 完成后才判定命中或失败。
  未知章节显示明确提示并停在文首。
- 同仓 Markdown 链接先解析完整 `repoPath`，再查注册表；保留 fragment。页内 `#` 交给笔记阅读器定位，
  不改写 SPA hash 路由。
- 笔记索引新增「W12 核心链」九份，不声称是 W12 全量笔记。概念地图、Bub 报告和 Demo 讲稿在展示状态可见；
  D2–D5、周计划与可视化方法稿只在复习状态可见。
- 复习材料仍会强制切到复习态；返回链接命中原专题并保持当前复习态，不额外恢复进入笔记前的展示态。

### 13.5 十列增量契约

| 列 | 内容 |
|---|---|
| ① 单一问题 | 概念地图为什么是这五个对象，它们如何连到当前八个专题与原始笔记？ |
| ② 结论锚 | 五张导航卡来自五周目标和 W12 材料的整理，经本人 review，集合开放；当前展板只提供允许重叠的阅读入口。 |
| ③ 对象与数据 | 原五节点七边；新增 3 个 provenance 步骤、5 组 `landingTopicIds/reason`、结构化 `NoteTarget` 与受限 `NoteReturnTarget`。 |
| ④ 结论编码 | provenance 是独立的形成轨；landing 是图下的可操作入口行；两者都不叠到七条概念边上。 |
| ⑤ 视觉舞台 | 形成轨 → 五节点七边图 → 当前展板阅读入口；节点可打开原文，入口按钮可切换专题。 |
| ⑥ 文字层级 | 常驻：形成过程边界、完整对象名、语义专题名、阅读入口边界。折叠：七边详细依据。 |
| ⑦ 记忆点 | 保留 Bub 交汇几何；阅读接线只负责从概念进到已建专题，不抢占原记忆点。 |
| ⑧ 图标策略 | 不增图标；清晰的文本链接与按钮承担导航。P/B 编号移出用户主路径。 |
| ⑨ 动效策略 | 保持原静态概念图；新导航只用 hover/focus 反馈，reduced-motion 下不丢信息。 |
| ⑩ 验收证据 | 既有五节点七边断言不删改；新验形成轨、八专题入口并集、双向总览、无裸编号、笔记注册/章节存在、主/细项真实点击、页内/跨笔记、刷新/显式返回、section 与返回参数清理、非法参数负控、完整路径解析；桌面/手机默认态、全展开态与人工四闸保留。 |

### 13.6 独立设计审查

- 第一轮发现了 topic id、mapping 语义、section 生命周期和对外短标签四个阻断，未进入实现。
- 修正为 `landingTopicIds`、有效 id、完整 section 状态机与八个冻结短标签后，第二轮结论为
  「无阻断性问题，可以实施」。

### 13.7 实现结果与证据边界

- 本轮基线为 `verify:board` 1363/1363、ai-engineer 288/288；当前为 **1482/1482**、
  ai-engineer **392/392**。新增断言直接检查 landing 集合、反向入口、结构化主/细项来源、真实章节、
  复习门前显式返回、页内/跨笔记、刷新、section 与返回参数生命周期、四组非法参数负控和完整路径匹配，
  没有删除或放宽 §12 的五节点七边断言。
- `yarn typecheck` 与 `yarn build:showcase` 通过，构建处理 372 个模块；`audit:visual` 采样 176 个
  视口专题状态，页面级横向溢出 0、控制台错误 0。
- 等价干净环境 `/tmp/nodejs-skillup-reading-return-clean.1ZuTh0/repo` 初始无依赖目录、Yarn 缓存和构建
  产物；`yarn install --immutable` 后 typecheck、372 模块构建、1482 项校验与 176 状态审计全部通过。
- 四张新截图位于 `/tmp/nodejs-skillup-w12-reading-links-2026-09-05/`。默认态和全展开态分别为
  `0/3`、`3/3` details；「混合」徽标、两条推断标记和 12 个阅读入口按钮均由 DOM 与截图复核。
- 手机全展开首轮暴露 65px 横向溢出；定位到关系依据层仍使用桌面三列布局，修复手机换行后复拍与
