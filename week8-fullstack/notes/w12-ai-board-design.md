...
| 块 | expected answer 命中 | 首屏结论锚 | 记忆点来自技术关系 | 记录时间 |
|---|---|---|---|---|
| P1 | 待填 | 待填 | 待填 | |
| P3 | 待填 | 待填 | 待填 | |
| B1 | 待填 | 待填 | 待填 | |
...
实现层决定（DOM 结构、class 与 `data-*` 命名、grid 列数、帧序编号、断言选择器）。一处需要显式记录的
细化：

- 上游 §4.4 P1 的数据契约写 `sides` 可含 ≥2 端点。本文件把「TS 端点的证据地位」写死为
  **定位用途**（本周实测发生在 Python 侧），并把该边界句放进常驻文字层，避免读者把六行读成
  「两侧都本周实测过」。这是补充说明，不改契约形状。
