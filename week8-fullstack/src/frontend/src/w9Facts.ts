 ...
 *    「放行前 timed out → 放行后 refused」的差分，安全组与 ufw 第一次被分别观察到。
...
/**
 * 安全组与 ufw 被分别观察到的那一刻。这是 8/13 最重要的一条实测——
...
export const LAYER_CHOICE = {
  ...
  costLater:
    "威胁模型不重叠，所以这不是「做了一层就够」。将来若公网需要合法 admin 访问 /users，统一的反代规则会误伤——届时要么开放规则并补鉴权，要么两层都做。",
...
export const SPOKEN_TAKEAWAYS = {
  ...
  stale:
    "按暴露渠道分，三关暴露的错性质不同：C1 暴露的是层内机制与形态，C2 暴露的是工具口误，而 C3 暴露的两条都是「记忆停在旧状态」——说的是曾经的样子或将来的规划，不是今天的事实。最后这类最难自查，因为它不是不懂原理，是心里那张图没跟着机器一起更新。这和展板给出与最新数据不一致的结论是同一种失效。",
} as const;
...
export interface Contract {
  id: string;
  what: string;
  /** 在哪一天通过验收；null = 到今天仍然欠着。 */
  settledOn: "D2" | "D3" | "D4" | "D5" | null;
  /**
   * 怎么结的。「做完了」与「决定不做」都算通过验收——它们都让这条不再悬着，
...
export const CONTRACTS: Contract[] = [
  ...
  },
  {
    ...
    settledOn: "D3",
    evidence: "B4 第二轮快失败注入 → restart counter at 5 → failed 停住 → 恢复 200。这是 D2 选 A 欠下的那条，补完即通过验收",
    grade: "measured",
  },
  {
    id: "reboot",
  ...
  },
  {
    ...
    settledOn: "D4",
    evidence:
      "8/13 同日通过验收，不必留到 D5：certbot.timer enabled + NEXT 8/14 04:13 CST + journal 有启动记录，并且 certbot renew --dry-run 实跑通过（all simulated renewals succeeded）——「会跑」有了证据，不再只是「应该会跑」",
    grade: "measured",
  },
  {
    id: "cold-path",
...
/**
 * D1 那 12 条到 8/14 全部通过验收之后，「还欠什么」这一问的答案换了一批人——
 * 不再是冻结清单里的欠账，而是这一周做事过程中新长出来的。
 *
 * 这一节必须存在，否则验收板会给出一个假结论：12 条全绿 = 没有欠账了。
