...
const common = { kind: "rag" as const, group: "RAG 成果" as const, evidenceKind: "混合" as const,
  ...
  sources: [
    { label: "实现与证据边界", ref: "week13-rag/notes/day4-full-context-baseline-and-bm25.md §6.14–§6.21" },
    { label: "冻结输入与检索职责", ref: "week13-rag/src/w13rag/{retrieval,generation,scoring}.py" },
  ] };
...
export const RAG_TOPICS: AeRagTopic[] = [
  ...
  { ...common, id: "rag-flow", label: "链路", title: "固定 RAG 链路", question: "当前系统如何从冻结语料走到回答？",
    anchor: "两种上下文输入，共用组装、生成与引用校验。", boundary: "固定 dev 链路已运行；质量门禁未通过。全量与检索路径的对照保留同一来源和输出契约。",
    memory: "全量与检索两支汇合到 Evidence Context。", accept: "指出 full-context 与 retrieval 的分叉和汇合，并说明 registry 校验 citation 的职责。" },
  { ...common, id: "rag-implementation", label: "实现理由", title: "为什么这样实现", question: "实现如何同时保留可追溯性、替换能力和评估边界？",
    ...
    sources: [{ label: "完整代码解读", ref: "rag-implementation-guide.md §1", target: { noteId: "w13ragguide", section: "1" } }],
  ...
  { ...common, id: "rag-evidence", label: "证据回放", title: "回答与证据回放", question: "回答里的 citation 如何定位到冻结原文？",
    anchor: "从回答点击引用，回到同一版本的原文行。", boundary: "2026-09-10 历史开发评测回放；页面不发起模型调用。BM25 人工语义判定待补，分支一致与引用可解析不能替代语义支持。",
  ...
  { ...common, id: "rag-eval", label: "检索对照", title: "检索与输入规模", question: "已有对照数据支持什么结论？",
    anchor: "检索在相同 8 题上比较；输入规模单独衡量。", boundary: "检索通过按 requirement span 交集判定；无答案 2 题不适用。输入 token 下降不代表质量提升或总费用下降。",
  ...
  { ...common, id: "rag-framework", label: "框架衔接", title: "框架职责映射", question: "现有经验怎样衔接 LangChain 与 LangGraph？",
    anchor: "LangChain 已承接 Document 与 BM25；LangGraph 编排留待后续实践。", boundary: "Dense 直接使用 ONNX + NumPy；生成复用此前阶段的模型客户端，尚未使用 LCEL。LangGraph 图为后续映射，尚未实施。",
