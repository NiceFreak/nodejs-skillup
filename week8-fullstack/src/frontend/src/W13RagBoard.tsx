// W13 RAG 输入工程的六块图。形态由内容关系推导，见
// week13-rag/notes/week13-visualization-plan.md §5 与返工记录 §3.7 / §3.7.1。
//
// 一条原则贯穿六块（§3.7 返工的结论）：主图画的是**对象本身**，量退成标注。
// 反例就在返工前——每块的图编码的是量与分类，结论讲的却是机制，两者不在回答同一个问题。
//   T0 = 一条真实内容走完五段的闭环（内容向右流，引用向左回）
//   T1 = 一份文件的三项「记录值 vs 复算值」两列对照 + 其余六份各三格
//   T2 = 真实源行逐行扫描 + 标题栈阶梯 + 块边界刻度
//   T3 = 真实 entry 逐层长大（主）+ 全语料合计瀑布（次）
//   T4 = 判分链指着响应 JSON 的被查部位走 + 右侧真实证据块
//   T5 = 把块改坏的实测：五种改法 × 两列「逐块 hash / 两块整串」变没变
//
// 承担结论的位置编码（改 CSS 前先看 scripts/verify-w9-board.mjs §F）：
//   T0 ⑤ 在 ① 正下方，走到末帧时 ① 里的核心行点亮 = 引用能走回同一行；
//      ② 登记 / ④ 引用 / ⑤ 回查必须是同一个 ID 字符串，断言直接查这三处相等；
//   T1 三帧的状态各不相同（未读取 / 已复算 / 已比对）= 校验真的分三步；两列并置 = 逐项比对；
//   T2 标题栈的缩进 = 语境层级，thematic break 那一行的上下虚线 = 不可跨越的硬边界；
//   T3 四帧只增不减 = 每一层都在上一层字节上包一圈；下降段起点在核心正文终点 = 有一段从不进入任何块；
//   T4 指针停在 branch 行 = 否决发生在分支判定处，不在链尾；
//   T5 出现「hash 没变 / 整串变了」的行 = 指纹盲区。这两列是导出脚本把块真改一遍算出来的，不是断言。
