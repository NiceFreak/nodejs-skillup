// W13 RAG 输入工程的六块（T0 总览 + T1–T5）。设计契约与形态推导见
// week13-rag/notes/week13-visualization-plan.md §5–§6、裁决见 §14、返工记录见 §3.7 / §3.7.1。
// 导航顺序跟随链路：总览 → ①输入冻结 → ②切分与引用 → ③组装与组成 → ⑤评测契约 → 贯穿的验证与证据。
// 总览排第一是因为它负责在图上用真实内容定义术语；其余五块都是它某一段的展开，
// 各自在页面顶部用 W13_STAGE_OF 标出自己对应哪一段。
//
