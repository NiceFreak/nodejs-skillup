...
> 日期：2026-07-24  
> 状态：D5 已验收；Worker 最小对比提前完成并通过，但 W5 总验收仍需等待 D6 的债务重建、脱稿串讲与四问复盘。  
...
两组都执行 `fib(40)`，HTTP 请求都等待计算结果后再响应；唯一核心差异是 CPU 计算的执行归属。

| 观测 | 空闲基线 | 主线程计算 | Worker 计算 |
|---|---:|---:|---:|
| 最大 heartbeat gap | 约 102ms | 1160ms | 102ms |
| 计算期间独立 `/ping` | 接近空闲 | 约 378ms | 约 2ms |
| 当前请求任务耗时 | 不适用 | 1126ms | 1106ms |

这些结果支持“Worker 保持主线程响应性”。单次任务耗时接近只描述本轮运行，不能证明 Worker 让计算更快。Worker 的 `message` / `error` / abnormal `exit` / timeout 已统一为单次响应出口，失败使用 500 或 504，避免把失败伪装成 200。
...
- 当前 Worker 对照中，主线程计算造成明显 heartbeat gap 与 `/ping` 延迟；Worker 组保持接近空闲基线。

### 受控推断

- Worker 的价值是改变 CPU 工作归属、保护主线程响应性，不是保证单任务更快。
...
- Worker 最小对比：AI 做 L1 实验契约与 review，核心 demo 由本人实现，不新增债务。
