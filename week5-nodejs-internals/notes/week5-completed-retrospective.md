...
> 截止：2026-07-24（2026-07-25 更正 Worker 部分）  
> **2026-07-25 更正**：本文原称「Worker 最小对比已提前通过」并把 Worker 作为主 demo。本人澄清 W5 D6 实际未进行学习，Worker 相关结论与数字作废，退回 D6（7/27）待完成；下文 Worker 内容保留但标注为「未验收」。  
> 口径：D1–D5 已完成；**D6 整体（含最小 Worker 对比）尚未完成**，债务重建、脱稿串讲与四问本人复盘也尚未完成。因此本文是“已完成内容复盘”，不是 W5 最终结课证明。
...
| ~~Worker~~（未验收 · 退回 D6） | —（原判断作废） | ~~`fib(40)`：heartbeat gap 1160ms→102ms；并发 `/ping` 378ms→2ms~~ · **2026-07-25 更正：该学习未真正进行，数字作废，退回 D6（7/27）** | 此行不作为已完成能力，仅保留以标明退回项 |
...
> **2026-07-25 更正**：原文以「主线程 vs Worker 对照」为主 demo，但该学习实际未进行、数字作废，不能再作为证据。以下改以背压 demo 为当前已验收的主 demo；Worker 对照留待 D6 真正完成后再评估是否作为主 demo。

背压 demo 作为当前主证据：它展示一类生产风险不是“主线程被阻塞”，而是 producer 与 consumer 速率不匹配。heartbeat 在暂停窗口仍继续，是排除主线程阻塞的关键对照，30 chunks 里 false / drain / `writableLength` 的实测都由本人跑过。

~~（原主 demo：主线程 vs Worker 对照——同一 CPU 工作、主线程 heartbeat、独立 `/ping` 三类证据。此对照未经真正学习验收，作废，退回 D6。）~~

## 范围取舍

10 分钟展示不逐项讲八个专题，也不现场跑所有脚本。

> **2026-07-25 更正**：原推荐四站以「Worker 对照」开场。Worker 对照未验收、退回 D6，暂不作为现场主站。D6 真正完成前，现场只走已验收三站：
...
事件循环、CPU timer、Stream 内存模型与 pipeline 作为三站背后的证据页，追问时再展开。~~Worker 对照（执行归属）~~ 待 D6 完成后再决定是否补回开场。这样既覆盖本周核心，又不会把展示变成名词巡游，也不把未验收内容当已验收讲。

## 未完成与风险

- D6 尚未完成，所以不能写“Week 5 已最终验收”。
- **最小 Worker 对比未完成**：此前误记已提前通过，2026-07-25 更正为未真正学习，退回 7/27 D6；`src/server.mjs`、`src/fib-worker.mjs` 仅作脚手架，不是掌握证据。
- `DEBT.md` 中 CPU timer 测量、threadpool 证据边界、异步 bcrypt 执行归属仍待 7/27 第一档重建。
- W4 的四条债务已重建通过但待补掌握证据，同样留到 D6。
- 当前 Worker 脚手架使用全局 heartbeat 状态，前提是串行实验；不能作为并发生产实现（且尚未经真正验收）。
