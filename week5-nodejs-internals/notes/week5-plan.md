# Week 5 计划 · Node.js 底层原理（7/20–7/27 收口）
...
> 2026-07-23 调整：D2 因临时面试与无效下钻跨日，原定 D3 主题顺延到 D4。D4 只学 Stream，D5 只学错误与进程生命周期。7/25 周六有提车与登记安排，时间被切碎，不安排强制学习；7/26 周日完整休息。D6 改为 W5 收口闸门，放到 7/27 的第一个完整专注块；通过后再进入 W6，7/31 硬截止不变。
...
- 能脱离展示板串讲至少三个运行时场景：事件循环、阻塞/线程池、stream 或 worker。
- 完成到期债务重建和四问复盘；不以展板或讲稿是否完成作为通过条件。
...
6. 能在 D6 脱离 AI、展示板和讲稿串讲一个“Node.js 底层理解 demo”。
...
- 白名单工具性内容可以由 AI 处理，例如目录整理、笔记体例整理、demo 讲稿、命令说明；其中展示和讲稿只能在对应结论通过本人验收后派生，不计入掌握证据。
...
- 7/23 已决定增加 D6，把错误生命周期与 Worker 拆开；7/25–7/26 不作为补习日，D6 顺延到 7/27 的首个完整专注块。若再发生中断，仍应顺延或砍展示范围，而不是把未掌握内容标为完成。
...
D5 做错误边界与 graceful shutdown；D6 作为掌握闸门，只做：
...
展示板润色、汇报 demo 和讲稿属于验收后的展示派生物，不进入 D6，也不要求在周末审核。7/22 不再开启 Stream；7/23 从 D4 S1 的工作场景进入。D4 不以覆盖术语数量验收，以能对真实后端文件链路作出实现、review 和排障判断为准。
